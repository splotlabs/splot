...
//! Region-verification tests for the frontier general-intra reconstruction bridge.
//!
...
//! fixture (the legacy local fixture env var or default local fixture path) and
//! `#[ignore]`d to match the existing `local_frontier_*` probe convention.
...
use super::reconstruct_frontier_intra_region_from_plan;
use crate::filters::wienerns_lr::WienerNsLrReconSink;
...
fn require_fixture() -> PathBuf {
    let Some(path) = local_frontier_path() else {
        panic!(
            "set the legacy local fixture env var or HOME for the ignored local frontier reconstruction test"
        );
    };
    assert!(
        path.is_file(),
        "local frontier fixture not found at {}",
        path.display()
    );
    path
}

/// Reads the local frontier fixture, plans it, and runs the selectable
/// transform-record walk, returning the reconstruction sink. The shared preamble
/// for every ignored frontier oracle-pin test (fixture → plan → reconstruct).
fn reconstruct_frontier_sink() -> WienerNsLrReconSink<u16> {
    let path = require_fixture();
    let bytes = std::fs::read(&path).expect("read frontier fixture");
    let options = DecodeOptions::default();
    let plan = context()
        .plan_bytes(&bytes, options)
        .expect("plan frontier");
    reconstruct_frontier_intra_region_from_plan(&bytes, &options, &plan, false)
        .expect("reconstruct frontier region")
}

/// As [`reconstruct_frontier_sink`], but drives the DIAGNOSTIC-ONLY full-reconstruction
...
/// the local full-reconstruction differential harness.
fn reconstruct_frontier_full_recon_sink() -> WienerNsLrReconSink<u16> {
    let path = require_fixture();
    let bytes = std::fs::read(&path).expect("read frontier fixture");
    let options = DecodeOptions::default();
    let plan = context()
        .plan_bytes(&bytes, options)
        .expect("plan frontier");
    reconstruct_frontier_intra_region_from_plan(&bytes, &options, &plan, true)
        .expect("full-recon frontier region")
}
...
/// primitive reuse) over the live frontier parse.
#[test]
#[ignore = "requires local mission fixture"]
fn frontier_reconstruction_bridge_populates_a_workspace_region() {
    let sink = reconstruct_frontier_sink();
...
/// frontier stream matches AVM `has_top_right` (`av2/common/reconintra.c:59`) at the
...
#[test]
#[ignore = "requires local mission fixture"]
fn frontier_per_transform_far_edge_matches_avm_has_top_right() {
    let sink = reconstruct_frontier_sink();
    assert_eq!(
...
/// checksum match the oracle. This is the first BIT-EXACT frontier reconstruction
/// milestone, verified against the AVM pre-filter oracle.
#[test]
#[ignore = "requires local mission fixture"]
fn frontier_frame_origin_dc_block_reconstructs_bit_exact_against_prefilter_oracle() {
    let sink = reconstruct_frontier_sink();
...
#[test]
#[ignore = "requires local mission fixture"]
fn frontier_block_16x64_luma_column_reconstructs_bit_exact_against_prefilter_oracle() {
    let sink = reconstruct_frontier_sink();
...
#[test]
#[ignore = "requires local mission fixture"]
fn frontier_frame_origin_chroma_dc_blocks_reconstruct_bit_exact_against_prefilter_oracle() {
    let sink = reconstruct_frontier_sink();
...
/// NON-SQUARE residual path, and (2) the §7.13.2.12 IBP DC modifier (frontier has
...
#[test]
#[ignore = "requires local mission fixture"]
fn frontier_first_three_superblock_luma_reconstructs_bit_exact_against_prefilter_oracle() {
    let sink = reconstruct_frontier_sink();
...
/// (non-DC) frontier block, and it unblocks the IntrABC brick (which copies from it).
#[test]
#[ignore = "requires local mission fixture"]
fn frontier_sb_column3_hpred_block_reconstructs_bit_exact_against_prefilter_oracle() {
    let sink = reconstruct_frontier_sink();
...
#[test]
#[ignore = "requires local mission fixture"]
fn frontier_frame_top_vpred_no_above_fallback_reconstructs_bit_exact_against_prefilter_oracle() {
    let sink = reconstruct_frontier_sink();
...
#[test]
#[ignore = "requires local mission fixture"]
fn frontier_paeth_two_sided_blocks_reconstruct_bit_exact_against_prefilter_oracle() {
    let sink = reconstruct_frontier_sink();
...
#[test]
#[ignore = "requires local mission fixture"]
fn frontier_first_intrabc_block_reconstructs_bit_exact_against_prefilter_oracle() {
    let sink = reconstruct_frontier_sink();
...
#[test]
#[ignore = "requires local mission fixture"]
fn frontier_first_intrabc_residual_block_reconstructs_bit_exact_against_prefilter_oracle() {
    let sink = reconstruct_frontier_sink();
...
#[test]
#[ignore = "requires local mission fixture"]
fn frontier_step8_admitted_region_reconstructs_bit_exact_against_prefilter_oracle() {
    let sink = reconstruct_frontier_sink();
...
#[test]
#[ignore = "requires local mission fixture"]
fn frontier_full_reconstructed_luma_region_matches_prefilter_oracle_aggregate() {
    let sink = reconstruct_frontier_sink();
...
#[test]
#[ignore = "requires local mission fixture"]
fn frontier_bottom_edge_partial_left_edge_block_reconstructs_bit_exact_against_prefilter_oracle() {
    let sink = reconstruct_frontier_sink();
...
/// proven no-edge-filter subset: frontier's `enable_intra_edge_filter == 1` but the
...
#[test]
#[ignore = "requires local mission fixture"]
fn frontier_one_sided_zone1_idif_block_reconstructs_bit_exact_against_prefilter_oracle() {
    let sink = reconstruct_frontier_sink();
    assert_luma_region_oracle(
        &sink,
...
#[test]
#[ignore = "requires local mission fixture"]
fn frontier_one_sided_edge_filter_corner_block_reconstructs_bit_exact_against_prefilter_oracle() {
    let sink = reconstruct_frontier_sink();
    assert_luma_region_oracle(
        &sink,
...
/// frontier luma plane width (the §6.4.2 `1920x1080` 10-bit 4:2:0 frame).
const FULL_FRAME_WIDTH: usize = 1920;
/// frontier luma plane height.
const FULL_FRAME_HEIGHT: usize = 1080;

/// Loads the AVM pre-filter oracle luma plane (the legacy AVM pre-filter env var or
...
///
/// Gated behind the local full-reconstruction env var (in addition to `#[ignore]`) so
/// a casual `--ignored` run that has no regenerated oracle does not fail; set the var
/// to run it: the local full-reconstruction env var before running the ignored test.
#[test]
...
fn frontier_full_decode_order_reconstruction_differs_against_prefilter_oracle() {
    ...
    for (idx, leaf) in sink.full_recon_luma_log().iter().enumerate() {
        ...
        if block_mismatch > 0 && first_clean_block.is_none() {
            let neighbours_exact = |leaf: &crate::filters::wienerns_lr::FullReconLumaLeaf| {
    ...
    eprintln!("==== frontier FULL DECODE-ORDER LUMA RECONSTRUCTION vs AVM pre-filter oracle ====");
    eprintln!(
