...
fn lossless_nondc_chroma_d157_frame_decodes_to_oracle() {
    assert_eq!(LOSSLESS_NONDC_CHROMA_D157_FIXTURE.len(), 552);
    let options = DecodeOptions::default();
    let context =
        DecodeContext::new(DecodeRuntimeConfig::new(ThreadCount::from(1usize))).expect("context");
    let plan = context
        .plan_bytes(LOSSLESS_NONDC_CHROMA_D157_FIXTURE, options)
        .expect("plan");
    let decoded = context
        .pool()
        .install(|| decode_frame_from_plan(LOSSLESS_NONDC_CHROMA_D157_FIXTURE, &options, &plan))
        .expect("decode");
