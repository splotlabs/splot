...
pub(crate) fn frame_for_output<'a, T: ReconSample>(
    ...
    apply_grain(frame, grain).map(|frame| DisplayFrame::Owned(Box::new(frame)))
}
...
fn apply_grain<T: ReconSample>(
    ...
    Ok(DecodedFrame::try_new(info, FramePlanes::new(y, u, v))?)
}
...
fn plane_from_visible<T: ReconSample>(
    ...
    Ok(Plane::from_vec(size, width, rect, converted)?)
}
...
fn build_noise_image(
    ...
    let plane_widths = plane_widths(width, sub_x, num_planes);
    let plane_heights = plane_heights(height, sub_y, num_planes);
...
fn plane_widths(width: usize, sub_x: usize, num_planes: usize) -> Vec<usize> {
    (0..num_planes)
        .map(|plane| {
            let plane_sub_x = if plane > 0 { sub_x } else { 0 };
            (width + plane_sub_x) >> plane_sub_x
        })
        .collect()
}

fn plane_heights(height: usize, sub_y: usize, num_planes: usize) -> Vec<usize> {
    (0..num_planes)
        .map(|plane| {
            let plane_sub_y = if plane > 0 { sub_y } else { 0 };
            (height + plane_sub_y) >> plane_sub_y
        })
        .collect()
}
