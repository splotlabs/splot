...
use crate::bitstream::tile_payload::{
    ...
    GeneralIntraResidualError, GeneralIntraTreeWalkError, TileCoeffContextState,
...
use crate::pipeline::{
    FRONTIER_LR_LIVE_TRANSFORM_RECORD_HANDOFF_FEATURE_ID,
    FRONTIER_LR_LIVE_TRANSFORM_RECORD_HANDOFF_MATRIX_ROW,
    FRONTIER_LR_RUNTIME_STORAGE_RETENTION_FEATURE_ID,
...
pub(crate) use self::live_storage::{
...
pub(crate) struct WienerNsLrSourceReadFrontier {
    pub(crate) blocks_resolved: usize,
    pub(crate) output_samples_resolved: usize,
    pub(crate) source_reads_resolved: usize,
    pub(crate) curr_frame_source_reads: usize,
    pub(crate) cdef_frame_source_reads: usize,
    pub(crate) first_sample: Option<WienerNsLrSourceReadSample>,
}

#[cfg_attr(not(test), allow(dead_code))]
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct WienerNsLrSourceReadSample {
    pub(crate) plane: PlaneId,
    pub(crate) x: usize,
    pub(crate) y: usize,
    pub(crate) source: LoopRestorationSource,
}

#[cfg_attr(not(test), allow(dead_code))]
#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub(crate) struct WienerNsLrClassifiedWienerFrontier {
    pub(crate) blocks_resolved: usize,
    pub(crate) feature_points_resolved: usize,
    pub(crate) source_reads_resolved: usize,
    pub(crate) curr_frame_source_reads: usize,
    pub(crate) cdef_frame_source_reads: usize,
    pub(crate) tx_skip_lookups_resolved: usize,
    pub(crate) first_sample: Option<WienerNsLrSourceReadSample>,
    pub(crate) first_tx_skip_lookup: Option<WienerNsLrTxSkipLookup>,
}
...
pub(crate) struct WienerNsLrClassifiedWienerStorageInputs<'a, T: ReconSample> {
    pub(crate) curr_frame: &'a DecodedFrame<T>,
    pub(crate) cdef_frame: &'a DecodedFrame<T>,
    pub(crate) tx_skip_grid: &'a WienerNsLrTxSkipGrid,
}

#[cfg_attr(not(test), allow(dead_code))]
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct WienerNsLrRuntimeStorageRetentionFrontier {
    pub(crate) bit_depth: BitDepth,
    pub(crate) frame_buffer_count: u64,
    pub(crate) frame_buffer_bytes: u64,
    pub(crate) retained_frame_buffer_bytes: u64,
    pub(crate) tx_skip_rows: usize,
    pub(crate) tx_skip_cols: usize,
    pub(crate) tx_skip_values: u64,
    pub(crate) total_storage_bytes: u64,
}

#[cfg_attr(not(test), allow(dead_code))]
#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub(crate) struct WienerNsLrClassifiedWienerValuesFrontier {
    pub(crate) blocks_resolved: usize,
    pub(crate) source_reads_resolved: usize,
    pub(crate) curr_frame_source_reads: usize,
    pub(crate) cdef_frame_source_reads: usize,
    pub(crate) filter_classes_resolved: usize,
    pub(crate) first_sample: Option<WienerNsLrClassifiedWienerValueSourceSample>,
    pub(crate) first_filter_class: Option<WienerNsLrFilterClassValue>,
}

#[cfg_attr(not(test), allow(dead_code))]
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct WienerNsLrFilterClassValue {
    pub(crate) x: usize,
    pub(crate) y: usize,
    pub(crate) row: usize,
    pub(crate) col: usize,
...
#[cfg_attr(not(test), allow(dead_code))]
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct WienerNsLrClassifiedWienerValueSourceSample {
    pub(crate) input_x: isize,
    pub(crate) input_y: isize,
    pub(crate) bounds: LoopRestorationSourceBounds,
    pub(crate) sample: WienerNsLrSourceReadSample,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct WienerNsLrSourceReadConfig {
    pub(crate) chroma_luma_source_taps: [[bool; WIENER_NS_CHROMA_SOURCE_TAP_COUNT]; 3],
...
pub(crate) const WIENER_NS_CHROMA_SOURCE_TAP_COUNT: usize = 12;
...
pub(crate) fn ensure_wienerns_lr_unit_tile_frontier(
    bytes: &[u8],
...
fn consume_wienerns_lr_unit_frontier(
    ...
    sequence: &SequenceHeader,
    core: &FrameHeaderCore,
) -> Result<crate::bitstream::tile_payload::TileLoopRestorationRootFrontier> {
    ...
    let tile = match tile_plan.work_units_mut() {
        ...
        [] => {
            return Err(unsupported_at(
                ...
                key_envelope.offset,
                "current supported tier requires one tile work unit before parsing LR unit syntax",
            ));
        }
        work_units => {
            return Err(unsupported_at(
                ...
                "current supported tier only consumes frontier LR unit syntax for one-tile key frames",
            ));
        }
    };
    crate::bitstream::tile_payload::consume_tile_lr_unit_frontier(
        tile,
        sequence,
        core,
        options.limits(),
...
pub(crate) fn derive_wienerns_lr_runtime_storage_retention_frontier(
    sequence: &SequenceHeader,
    core: &FrameHeaderCore,
    offset: ByteOffset,
    limits: DecodeLimits,
    ...
    let frame_size = core.frame_size.ok_or_else(|| {
        unsupported_feature_at(
            ...
            offset,
            "decode pipeline cannot retain loop-restoration storage before the parsed frame size is available",
            FRONTIER_LR_RUNTIME_STORAGE_RETENTION_MATRIX_ROW,
            FRONTIER_LR_RUNTIME_STORAGE_RETENTION_FEATURE_ID,
            "6.17.4.1",
        )
    })?;
    ...
    let (tx_skip_rows, tx_skip_cols) = crate::bitstream::tile_payload::frame_mi_dimensions(core)
...
pub(crate) fn derive_wienerns_lr_live_storage_allocation(
...
fn derive_wienerns_lr_fixed_largest_transform_record_handoff(
    ...
    let symbols = decode_general_intra_multiblock_tree(
        ...
        |work_unit, symbols, frontier, joint_modes, uses_mrls, fsc_modes, palette_state, is_cfl_ctx, _block_decoded| {
            ...
            Ok(crate::bitstream::tile_payload::GeneralIntraLeafMode::luma(
...
pub(crate) fn populate_wienerns_lr_live_tx_skip_from_transform_records(
...
impl WienerNsLrTransformRecordDiagnosticScope {
    const fn matrix_row(self) -> &'static str {
        match self {
            Self::FixedLargest => FRONTIER_LR_LIVE_TRANSFORM_RECORD_HANDOFF_MATRIX_ROW,
            Self::Selectable => FRONTIER_SELECTABLE_TRANSFORM_RECORDS_MATRIX_ROW,
        }
    }

    const fn feature_id(self) -> &'static str {
        match self {
            Self::FixedLargest => FRONTIER_LR_LIVE_TRANSFORM_RECORD_HANDOFF_FEATURE_ID,
            Self::Selectable => FRONTIER_SELECTABLE_TRANSFORM_RECORDS_FEATURE_ID,
        }
    }
}
...
fn wienerns_lr_transform_record_setup_error(
    ...
    match error {
        ...
        TilePartitionFrontierError::Traversal(error) => {
            wienerns_lr_transform_record_traversal_error(error, offset, scope)
        }
        TilePartitionFrontierError::MissingFact { .. } => wienerns_lr_transform_record_unsupported(
            scope,
        ...
        TilePartitionFrontierError::MiSizeState(_) => wienerns_lr_transform_record_unsupported(
            scope,
        ...
        TilePartitionFrontierError::UsesMrlsState(_) => wienerns_lr_transform_record_unsupported(
            scope,
        ...
        TilePartitionFrontierError::FscModeState(_) => wienerns_lr_transform_record_unsupported(
            scope,
        ...
        TilePartitionFrontierError::UvCflState(_) => wienerns_lr_transform_record_unsupported(
            scope,
...
pub(crate) fn wienerns_lr_source_read_config(
...
pub(crate) fn derive_wienerns_lr_runtime_source_frontiers(
    active_source_blocks: &[crate::bitstream::tile_payload::WienerNsLrSourceBlock],
    planes: &[LrPlaneParams],
...
#[cfg(test)]
pub(crate) fn derive_wienerns_lr_classified_wiener_frontier(
    active_source_blocks: &[crate::bitstream::tile_payload::WienerNsLrSourceBlock],
    planes: &[LrPlaneParams],
    limits: DecodeLimits,
...
#[cfg_attr(not(test), allow(dead_code))]
pub(crate) fn derive_wienerns_lr_classified_wiener_values_frontier<T, FS, FT>(
    active_source_blocks: &[crate::bitstream::tile_payload::WienerNsLrSourceBlock],
    planes: &[LrPlaneParams],
    bit_depth: BitDepth,
    base_q_idx: u32,
...
#[cfg_attr(not(test), allow(dead_code))]
pub(crate) fn derive_wienerns_lr_classified_wiener_storage_frontier<T>(
    active_source_blocks: &[crate::bitstream::tile_payload::WienerNsLrSourceBlock],
    planes: &[LrPlaneParams],
    bit_depth: BitDepth,
    base_q_idx: u32,
...
#[cfg(test)]
pub(crate) fn derive_wienerns_lr_source_read_frontier(
    active_source_blocks: &[crate::bitstream::tile_payload::WienerNsLrSourceBlock],
    chroma_format: ChromaFormatIdc,
    config: WienerNsLrSourceReadConfig,
    offset: ByteOffset,
...
pub(crate) fn record_wienerns_lr_chroma_luma_source_reads(
    summary: &mut WienerNsLrSourceReadFrontier,
    config: WienerNsLrSourceReadConfig,
    chroma_x: isize,
    chroma_y: isize,
