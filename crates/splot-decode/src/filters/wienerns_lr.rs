...
pub(crate) struct FrameFilterRecords {
    ...
    /// The decode's plane pool, so a workspace built from `retired_planes`
    /// knows where its storage goes when the last filter job releases it.
    pub(crate) planes: Option<std::sync::Arc<splot_recon::PlanePool>>,
}
