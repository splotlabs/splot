...
//! Unit tests for the frontier selectable reconstruction sink ([`super`]).
...
fn ist_nonzero_dc_leaf_is_deferred() {
    ...
    block.intra_ist = Some(crate::bitstream::tile_payload::IntraIstSyntax {
        sec_tx_type: 1,
...
fn intrabc_non_skip_residual_leaf_with_real_ist_is_deferred() {
    ...
    .unwrap();
    let mut residual = coeff_block_16x16();
    residual.intra_ist = Some(crate::bitstream::tile_payload::IntraIstSyntax {
        sec_tx_type: 1,
        most_probable_stx_set: Some(0),
    });
    sink.reconstruct_luma_transform(
