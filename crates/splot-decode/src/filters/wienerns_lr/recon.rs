...
//! frontier general-intra reconstruction bridge.
//!
//! Feature tracking: the selectable-transform-record frontier row.
//!
...
//! [`crate::pipeline::reconstruct`] §7.13.2 / §7.14.4 / §7.15.4 / §7.14.3
//! primitives — exactly the prediction→residual→write pattern `general_intra.rs`
//! uses, but driven from the live frontier walk instead of a synthetic fixture.
//!
...
pub(crate) struct SelectableReconContext {
    pub(crate) leaf_y_mode: Option<IntraYMode>,
    ...
    pub(crate) directional_luma: Option<SupportedDirectionalLumaMode>,
    ...
    pub(crate) chroma_mode: Option<SupportedChromaMode>,
    pub(crate) cfl_params: Option<CflParams>,
    ...
    /// coverage).
    pub(crate) is_intrabc: bool,
}

/// Reconstructs the verified NON-IntrABC general-intra DC subset of the frontier
...
pub(crate) struct WienerNsLrReconSink<T: ReconSample> {
    ...
    /// modifier when this is set. (frontier's sequence enables IBP.)
    enable_ibp: bool,
    ...
    /// (frontier's sequence sets this flag.)
    ...
    /// driven only by the the local full-reconstruction ignored harness). When `true`,
    ...
    /// otherwise). The local full-reconstruction harness replays it to find the FIRST
...
pub(crate) struct FullReconLumaLeaf {
    pub mi_col: usize,
    pub mi_row: usize,
    pub x: usize,
    pub y: usize,
...
impl<T: ReconSample> WienerNsLrReconSink<T> {
    /// Allocates a sink whose workspace is sized to the frontier frame (a positive
    ...
    #[allow(dead_code)]
    pub(crate) fn into_frame(mut self) -> Result<DecodedFrame<T>> {
    ...
    #[allow(clippy::too_many_arguments)]
    pub(crate) fn record_deblock_block(
        &mut self,
        mi_col: usize,
        mi_row: usize,
        block_col: usize,
        ...
        self.deblock_blocks
            .push(crate::filters::deblock::DeblockBlock {
                r: mi_row,
                c: mi_col,
                block_r: block_row,
                block_c: block_col,
    ...
    pub(crate) fn record_chroma_deblock_block(
        &mut self,
        plane_id: PlaneId,
        x: usize,
        y: usize,
    ...
    pub(crate) fn record_block_decoded_far_edge(
        &mut self,
        mi_col: usize,
        mi_row: usize,
        tx_size: usize,
    ...
    #[cfg(test)]
    pub(crate) fn block_decoded_far_edge(
        &self,
        mi_col: usize,
        mi_row: usize,
    ) -> Option<(usize, usize)> {
    ...
    /// §5.20.6 `TxSize` index.
    #[allow(clippy::too_many_arguments)]
    pub(crate) fn reconstruct_luma_transform(
        &mut self,
        mi_col: usize,
        mi_row: usize,
        tx_size: usize,
    ...
    #[allow(clippy::too_many_arguments)]
    pub(crate) fn reconstruct_chroma_transform(
        &mut self,
        plane_id: PlaneId,
        chroma_tx: usize,
        x: usize,
    ...
    pub(crate) fn reconstruct_intrabc_block(
        &mut self,
        source: PlaneRect,
        target: PlaneRect,
    ...
    #[cfg(test)]
    pub(crate) fn reconstructed_sample(&self, plane_id: PlaneId, x: usize, y: usize) -> Result<T> {
    ...
    #[cfg(test)]
    pub(crate) const fn reconstructed_counts(&self) -> (usize, usize) {
    ...
    /// visited.
    #[cfg(test)]
    pub(crate) fn for_each_reconstructed_luma_sample(
        &self,
...
/// Drives the frontier `TX_MODE_SELECT` selectable transform-record walk with a
...
pub(crate) fn reconstruct_frontier_selectable_intra_region(
    ...
    let frame_size = core.frame_size.ok_or_else(|| {
        crate::pipeline::unsupported_at(
            ...
            key_envelope.offset,
            "frontier reconstruction bridge requires the parsed frame size",
        )
    })?;
    let bit_depth = BitDepth::from_av2_bit_depth_idc(sequence.general.bit_depth_idc.get())?;
    crate::pipeline::ensure_runtime_limits(
        options.limits(),
        frame_size.width,
        frame_size.height,
        0,
...
/// The single fail-closed reason the frontier selectable walk is expected to stop on
...
pub(crate) fn luma_sample_origin(
    mi_col: usize,
    mi_row: usize,
    tile_offset: ByteOffset,
) -> Result<(usize, usize)> {
...
/// frontier `TX_16X64` `DC_PRED` leaf at MI(4,0) (x[16,32), y[0,64)), and an `eob > 1`
