...
//! Full-reconstruction support for the frontier sink.
//!
//! Full-reconstruction support for the frontier sink: a per-leaf decode-order log and
...
use crate::bitstream::tile_payload::{
    GeneralIntraResidualError, IntraYMode, SupportedDirectionalLumaMode,
};
...
pub(crate) fn full_recon_deferred_leaf_error(offset: ByteOffset) -> crate::error::DecodeError {
...
impl<T: ReconSample> WienerNsLrReconSink<T> {
    ...
    pub(crate) const fn is_full_recon(&self) -> bool {
        self.full_recon
    }
    ...
    pub(crate) fn reconstruct_intrabc_fractional_into(
        &mut self,
        target: PlaneRect,
        scaling: PlaneScaling,
    ...
    /// once.
    pub(crate) fn full_recon_far_edge(
        &self,
        mi_col: usize,
        mi_row: usize,
        side: FarEdgeSide,
    ...
    pub(crate) fn record_full_recon_leaf(
        &mut self,
        mi_col: usize,
        mi_row: usize,
        log2_width: u32,
    ...
    /// a gated sink). The local full-reconstruction harness replays this to locate the
    /// FIRST decode-order block whose samples diverge from the AVM oracle.
    #[cfg(test)]
    pub(crate) fn full_recon_luma_log(&self) -> &[FullReconLumaLeaf] {
    ...
    pub(crate) fn defer_full_recon_leaf(
        &mut self,
        mi_col: usize,
        mi_row: usize,
        log2_width: u32,
    ...
    pub(crate) fn finish_luma_predict(
        &mut self,
    ...
    pub(crate) fn routed_angular_wrote(
        &self,
        routed: Result<bool>,
        tile_offset: ByteOffset,
    ) -> Result<bool> {
...
pub(crate) struct OneSidedEdgeSpec {
...
/// never remapped.
pub(crate) fn wide_angle_mapping(w: u32, h: u32, p_angle: i32) -> i32 {
    let w = w as i32;
    let h = h as i32;
...
/// mode. Used ONLY by the local full-reconstruction harness reporting.
pub(crate) fn full_recon_mode_label(
