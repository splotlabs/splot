...
#[test]
fn class_three_width_sixteen_matches_scalar_samples_for_all_tables() {
    ...
    for bit_depth in [BitDepth::Eight, BitDepth::Ten] {
        ...
        for (ref_dst_idx, alpha_by_qp) in GDF_ALPHA.iter().enumerate() {
            for qp_idx in 0..alpha_by_qp.len() {
                ...
                let wide_result = gdf_uniform_width_rows::<16, 3, 1>(
                    [base_luma],
                    &source,
                    &tap_offsets,
                    &classes,
