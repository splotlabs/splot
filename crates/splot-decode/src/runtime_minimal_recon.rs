...
/// - `haveLeft && haveAbove`: `LeftCol` is the reconstructed left column, `AboveRow`
///   the reconstructed above row, and the corner is `CurrFrame[plane][y-1][x-1]`,
///   which `intra_dc_edges_for_rect` does not return, so it is reconstructed from
///   the workspace by the caller path that has both edges (not exercised by the
///   committed fixture, whose D135 block is row 0 with `haveAbove == 0`); this
///   helper approximates the corner with the first above sample, matching the
///   `haveLeft && !haveAbove` fixture path and the no-corner-read D135 geometry.
/// - `haveLeft && !haveAbove` (the committed fixture's row-0 D135 block):
///   `LeftCol[i]` is the reconstructed left column (clamped at the bottom-left
///   sentinel, `num4BelowLeft == 0` in raster order), `AboveRow[i]` is the repeated
///   first left sample `CurrFrame[plane][y][x-1]`, and the corner
///   `AboveRow[-1] = LeftCol[-1] = CurrFrame[plane][y][x-1]` (the first left sample).
/// - `!haveLeft && haveAbove`: symmetric (above row real, left column the repeated
///   first above sample, corner the first above sample).
...
fn build_directional_middle_edges(
    ...
    match (have_left, have_above) {
        (true, true) => {
            ...
            // §7.13.2.1: corner AboveRow[-1] = LeftCol[-1] = CurrFrame[plane][y-1][x-1].
            // `intra_dc_edges_for_rect` returns LeftCol[0..) and AboveRow[0..) but
            // not the (x-1, y-1) corner; for D135 the corner is never read (the
            // above branch reads AboveRow[j-i-1] for j>i so base>=0, the left branch
            // reads LeftCol[i-j-1] for j<i so base>=0, and the corner AboveRow[-1]
            // is read only at base==-1 which D135 does not reach over both edges).
            // Use the first above sample as a faithful, never-read placeholder.
            let corner = above_samples
                .first()
                .copied()
                .unwrap_or(NONEIGHBOUR_CORNER_8BIT);
            left.push(corner);
            above.push(corner);
            for i in 0..side {
        ...
        (false, true) => {
            // §7.13.2.1 !haveLeft && haveAbove: LeftCol[i] = CurrFrame[plane][y-1][x]
            // (the first above sample, repeated), corner = first above sample.
            let above_samples = above_neighbour.unwrap_or(&[]);
