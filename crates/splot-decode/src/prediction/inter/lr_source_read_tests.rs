...
use crate::bitstream::tile_payload::WienerNsLrSourceBlock;
use crate::error::DecodeError;
use crate::filters::wienerns_lr as lr;
use crate::filters::wienerns_lr::WienerNsLrClassifiedWienerValuesFrontier;
...
fn tx_skip_grid(values: Vec<u8>) -> lr::WienerNsLrTxSkipGrid {
    lr::WienerNsLrTxSkipGrid::new(4, 4, values).unwrap()
}

fn storage_inputs<'a>(
    curr_frame: &'a DecodedFrame<u16>,
    cdef_frame: &'a DecodedFrame<u16>,
    tx_skip_grid: &'a lr::WienerNsLrTxSkipGrid,
) -> lr::WienerNsLrClassifiedWienerStorageInputs<'a, u16> {
    lr::WienerNsLrClassifiedWienerStorageInputs {
        curr_frame,
        cdef_frame,
        tx_skip_grid,
    }
...
fn classified_wiener_storage_frontier(
    tx_skip: &lr::WienerNsLrTxSkipGrid,
    ...
    lr::derive_wienerns_lr_classified_wiener_storage_frontier(
        &blocks,
        &planes,
        BitDepth::Ten,
        0,
...
fn wienerns_lr_source_read_frontier_skips_zero_chroma_luma_taps() {
    ...
    config.chroma_luma_source_taps[PlaneId::U.index()] =
        [false; lr::WIENER_NS_CHROMA_SOURCE_TAP_COUNT];
...
fn wienerns_lr_source_read_config_uses_parsed_chroma_coefficients() {
    ...
    let config = crate::filters::wienerns_lr::wienerns_lr_source_read_config(&planes, 2);
    ...
    assert_eq!(
        ...
        [true; lr::WIENER_NS_CHROMA_SOURCE_TAP_COUNT]
    );
}
...
fn wienerns_lr_source_read_frontier_uses_frame_y_bound_for_chroma_luma_reads() {
    ...
    let mut summary = lr::WienerNsLrSourceReadFrontier {
        blocks_resolved: 0,
    ...
    lr::record_wienerns_lr_chroma_luma_source_reads(
        &mut summary,
        wienerns_lr_source_read_config(),
        0,
        7,
...
fn wienerns_lr_source_read_frontier_rejects_unit_coded_chroma_before_source_reads() {
    ...
    let config = crate::filters::wienerns_lr::wienerns_lr_source_read_config(&planes, 0);
    let limits = DecodeLimits::unlimited()
        .with_max_loop_restoration_source_reads(DecodeLimitThreshold::Max(0));

    let error = lr::derive_wienerns_lr_runtime_source_frontiers(
        &blocks,
        &planes,
        ChromaFormatIdc::Yuv420,
        config,
...
fn wienerns_lr_classified_wiener_frontier_resolves_dependency_coordinates() {
    ...
    let frontier = lr::derive_wienerns_lr_classified_wiener_frontier(
        &blocks,
        &planes,
        DecodeLimits::unlimited(),
    )
    ...
    assert_eq!(
        ...
        Some(lr::WienerNsLrTxSkipLookup {
            x: 0,
            y: 8,
            row: 2,
            col: 0,
...
fn wienerns_lr_classified_wiener_values_frontier_derives_filter_class() {
    ...
    let frontier = lr::derive_wienerns_lr_classified_wiener_values_frontier::<u8, _, _>(
        &blocks,
        &planes,
        BitDepth::Eight,
        0,
    ...
    assert_eq!(
        first_source.get(),
        Some(lr::WienerNsLrClassifiedWienerValueSourceSample {
            input_x: -1,
            input_y: 5,
            ...
            sample: lr::WienerNsLrSourceReadSample {
                plane: PlaneId::Y,
                x: 0,
                y: 6,
                source: LoopRestorationSource::CurrFrame,
    ...
    assert_eq!(
        ...
        Some(lr::WienerNsLrFilterClassValue {
            x: 0,
            y: 6,
            row: 1,
            col: 0,
...
fn wienerns_lr_classified_wiener_values_frontier_preflights_limit_before_reads() {
    ...
    let error = lr::derive_wienerns_lr_classified_wiener_values_frontier::<u8, _, _>(
        &blocks,
        &planes,
        BitDepth::Eight,
        0,
...
fn wienerns_lr_tx_skip_grid_retention_derives_spec_values() {
    ...
    let grid = lr::derive_wienerns_lr_tx_skip_grid_retention(2, 3, &records)
        .expect("complete LrTxSkip grid");

    let value = |row, col| {
        grid.lookup(lr::WienerNsLrTxSkipLookup {
            x: col * 4,
            y: row * 4,
            row,
            col,
...
fn wienerns_lr_tx_skip_grid_retention_rejects_missing_cells() {
    let records = [lr::WienerNsLrTxSkipTransformRecord {
        row: 0,
        col: 0,
        rows: 1,
        cols: 1,
    ...
    let error = lr::derive_wienerns_lr_tx_skip_grid_retention(2, 2, &records)
...
fn wienerns_lr_tx_skip_grid_retention_rejects_conflicting_records() {
    ...
    let error = lr::derive_wienerns_lr_tx_skip_grid_retention(1, 1, &records)
...
fn wienerns_lr_classified_wiener_storage_frontier_propagates_tx_skip_grid_bounds() {
    let tx_skip = lr::WienerNsLrTxSkipGrid::new(1, 1, vec![0]).unwrap();
...
fn classified_wiener_storage_runtime_error_reports_retention_frontier() {
    let error = crate::filters::wienerns_lr::wienerns_lr_classified_wiener_storage_runtime_error(
        ByteOffset::new(74),
    );
    assert_unsupported_feature(
        ...
        UnsupportedFeatureExpectation::at_byte_offset(
            ...
            concat!("ac0", "ej3-", "lr-classified-wiener-storage"),
            concat!("DECODE-", "AC0", "EJ3-", "LR-CLASSIFIED-WIENER-STORAGE"),
            "7.20.4",
            ByteOffset::new(74),
            &[
                "source-read",
...
fn wienerns_lr_runtime_storage_retention_frontier_counts_ten_bit_buffers_and_tx_skip_grid() {
    ...
    assert_eq!(
        ...
        64 * 64 * 3 / 2 * lr::LR_LIVE_FRAME_SAMPLE_STORAGE_BYTES * 2
    );
    ...
    assert_eq!(
        ...
        frontier.retained_frame_buffer_bytes
            + frontier.tx_skip_values * lr::LR_LIVE_TX_SKIP_STORAGE_BYTES_PER_VALUE
    );
}
...
fn wienerns_lr_runtime_storage_retention_error_reports_unpopulated_boundary() {
    let error = crate::filters::wienerns_lr::wienerns_lr_runtime_storage_retention_error(
        ByteOffset::new(74),
    );
    assert_unsupported_feature(
        error,
        ...
        UnsupportedFeatureExpectation::at_byte_offset(
            ...
            concat!("ac0", "ej3-", "lr-runtime-storage-retention"),
            concat!("DECODE-", "AC0", "EJ3-", "LR-RUNTIME-STORAGE-RETENTION"),
            "7.20.4",
            ByteOffset::new(74),
            &[
...
#[test]
fn wienerns_lr_tile_frontier_preflights_classified_and_filter_reads_together() {
    let blocks = [wienerns_lr_source_block()];
    let planes = [lr_plane(true, Some(2), None)];
    let limits = DecodeLimits::unlimited()
