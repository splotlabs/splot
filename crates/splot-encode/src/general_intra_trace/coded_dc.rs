...
use super::emit_minimal_intra_ivf;
...
/// The luma DC magnitude is `6` (`coeff_base_eob` saturated at
...
/// would need the currently unsupported tail.
///
...
/// negative level-6 DC coefficient (`txb_skip == 0`, `eob_pt == 0`, `coeff_base_eob`,
/// `coeff_br`, `dc_sign`) at the general `TX_64X64` contexts,
...
/// negative level-6 DC) and flat `128` chroma. It is the
...
pub fn emit_minimal_intra_coded_dc_ivf() -> Result<Vec<u8>> {
    let trace = compose_general_intra_dc_coded_block_trace()?;
    emit_minimal_intra_ivf(&trace)
}
...
mod tests {
    ...
    fn composes_general_coded_block_trace_in_order() {
        let trace = compose_general_intra_dc_coded_block_trace().unwrap();
