...
/// when signalled) is truncated, or [`Error::InvalidObuHeader`] if `start` is out
/// of range.
pub fn read_obu_header(input: &[u8], start: ByteOffset) -> Result<ObuHeader> {
    ...
    let mut reader = BitReader::new(buf, start);
    let has_header_extension = reader.read_bit()? == 1;
    let obu_type = ObuType::from_raw(reader.read_bits_u8(5)?);
    let temporal_layer_id = TemporalLayerId::new(reader.read_bits_u8(2)?);

    let (embedded_layer_id, extended_layer_id, header_size_bytes) = if has_header_extension {
        let embedded = EmbeddedLayerId::new(reader.read_bits_u8(3)?);
        let extended = ExtendedLayerId::new(reader.read_bits_u8(5)?);
        (embedded, extended, 2)
    } else {
        let extended = if obu_type.requires_global_xlayer() {
            GLOBAL_XLAYER_ID
        } else {
            ExtendedLayerId::new(0)
        };
        (EmbeddedLayerId::new(0), extended, 1)
    };
