...
mod tests {
    ...
    /// MSB-first bit writer for building film-grain payloads in tests (mirrors the parser's `Bits`
    /// helper, so this module reuses the same hand-built, spec-grounded fixtures).
    #[derive(Default)]
    struct Bits {
        bits: Vec<u8>,
    }
