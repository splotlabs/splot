...
mod tests {
    ...
    /// MSB-first bit builder mirroring the §5.8 parser test's own helper, so this module reuses the
    /// same hand-built, spec-grounded fixtures.
    #[derive(Default)]
    struct Bits {
        bits: Vec<u8>,
    }
    ...
    impl Bits {
        ...
        /// Appends a single-byte `leb128()` for `value` (`value < 128`).
        fn leb128_byte(&mut self, value: u32) {
            assert!(value < 128, "test helper only encodes single-byte leb128");
            self.f(value, 8);
        }

        /// Appends one `lcr_seq_profile_tier_level_info()` (§ 5.8.4) body.
        fn ptl(&mut self, profile: u32, level: u32, tier: u8, mlayer: u32, reserved: u32) {
            self.f(profile, 5);
            self.f(level, 5);
            self.bit(tier);
            self.f(mlayer, 3);
