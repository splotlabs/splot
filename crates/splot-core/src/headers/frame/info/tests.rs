...
fn frame_header_core_intra_tail_parses_full_structure_cluster() {
    ...
    assert_eq!((cdef.cdef_strengths, cdef.strengths.len()), (Some(1), 1));
    assert_eq!(cdef.cdef_on_skip_txfm_frame_enable, Some(true));
    let strength = &cdef.strengths.as_slice()[0];
    assert_eq!(strength.y_pri_strength, 9);
    assert_eq!(strength.y_sec_strength, 1);
    assert_eq!(strength.uv_pri_strength, 0);
    assert_eq!(strength.uv_sec_strength, 4);
    let lr = core.lr_params.as_ref().unwrap();
    assert!(!lr.uses_lr);
