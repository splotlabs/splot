...
pub fn parse_global_motion_params(
    ...
    let use_global_motion = reader.read_bit()? != 0;
...
pub fn decode_subexp(reader: &mut BitReader<'_>, num_syms: i64, k: u32) -> Result<i64> {
    ...
    loop {
        ...
        let more_bits = reader.read_bit()? != 0;
        if more_bits {
...
mod tests {
    ...
    impl Bits {
        ...
        /// Pushes the raw bit string (`'0'`/`'1'`) — keeps a hand-computed vector readable
        /// next to the trace comment.
