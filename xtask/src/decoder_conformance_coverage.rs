...
const COVERAGE_ROWS: &[CoverageRow] = &[
    ...
    CoverageRow {
        ...
        decoder_support_rows: &[
            ...
            "recon-reference-frame-store-fuzz",
        ],
        feature_ids: &[
            "AV2-8.2-SYMBOL-DECODER",
            "DECODE-TILE-CDF-SELECTION-BOUNDARY",
            "RECON-REFERENCE-FRAME-STORE",
            "CONF-RECON-REFERENCE-FRAME-STORE-FUZZ",
        ],
        status: "unsupported",
        tests: &["cargo test -p splot-recon reference --locked"],
        fuzz_targets: &["recon_reference_frame_store_bytes"],
        local_reference_evidence: &[],
        diagnostics: &["decode/unsupported-feature"],
        notes: "The current implementation has a partition CDF boundary subset and a generic reference store container with no-panic fuzz coverage over bounded storage API operation sequences, but no full runtime CDF lifecycle, layer context management, reference list construction, reference refresh semantics, motion-field estimation, or motion-vector prediction.",
    },
    CoverageRow {
    ...
    CoverageRow {
        ...
        local_reference_evidence: &[],
        diagnostics: &["decode/unsupported-feature"],
        notes: "Frame/hash/Y4M/reference primitives exist for caller-supplied frames, with hash input and Y4M serialization fuzzed from bounded structured frames and the generic reference store fuzzed from bounded storage operation sequences, and the documented minimal runtime tier emits hash/Y4M output for one supported fixture shape. The runtime hash/Y4M byte APIs are fuzzed with bounded raw bytes, minimal fixture mutations, and in-memory writers. Broad runtime output ordering, post-film-grain output, show-existing/flush behavior, motion-field storage, and AV2 reference refresh semantics remain unsupported. Current raw AVM/dav2d MD5 metadata is background evidence, not runtime coverage proof.",
    },
    CoverageRow {
