...
fn emitted_ids_for(root: &Path, registry: &RegistryConfig) -> Result<BTreeSet<String>> {
    ...
    for rel_root in registry.source_roots {
        ...
        for path in files {
            ...
            // Generated files (e.g. the `gen-explain` registry) are committed under a
            // scanned source root but only re-list already-documented ids; counting
            // them as "emitted" would mask a documented-but-unemitted id, so skip them.
...
/// `true` when `text` is an xtask-generated source file (identified by the
/// `@generated by `cargo xtask …`` header marker the generators emit).
...
mod tests {
    ...
    #[test]
    fn generated_source_is_detected_by_header_marker() {
        let generated = "// SPDX-License-Identifier: x\n\n//! @generated by `cargo xtask gen-explain` from …\n\nconst R: &[&str] = &[\"ops/foo\"];\n";
        assert!(is_generated_source(generated));
        // A hand-written source that merely mentions the word generated is not skipped.
        assert!(!is_generated_source(
            "fn f() { d(\"ops/foo\"); } // a generated-looking id\n"
        ));
    }
