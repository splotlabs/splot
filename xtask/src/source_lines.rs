...
const HARD_LINE_ALLOWANCES: &[SourceLineAllowance] = &[
    ...
    SourceLineAllowance {
        ...
        max_lines: 5_120,
        reason: "large cohesive frame-header parser; split separately from this validator refactor. +16 (2026-06-15): the maintainer-approved frame-config discarded-bit model extension adds FrameHeaderCore.force_integer_mv / .intrabc so the byte-exact §5.18.3 frame-header writer can reproduce every read-and-discarded bit (frame-header-writer-size-config). +16 (2026-06-15): expose CoreSeqView / MfhFrameView / CoreSeqInterView (pub, crate-private fields) + their from_sequence/from_record constructors and pub(crate) init_core_from_prefix/parse_core_body so the composing intra frame-header writer (write_frame_header_core, frame-header-writer-compose) can take them as inputs and round-trip in its sibling tests. +409 (2026-06-16): the single-picture IsBridge parse fix adds parse_single_picture_bridge_tail (the spec-mirror prefix, the overwrite-gated refresh per §6.17.2+AVM, the decidable §5.18.10.1 film-grain tail, and the BruInactiveOrBridgeReturn stop) and its five positive/data-dependent/EOF tests, replacing the buggy intra-key-path test (frame-header-single-picture-bridge-fix). +41 (2026-06-20): CoreSeqInterView::new_minimal_intra() — the public encoder writer-input constructor for the all-disabled §5.4.6 inter view (the one #[non_exhaustive] nested CoreSeqView sub-view that lacked a public ctor) — plus its all-disabled field test, so splot-encode can build a CoreSeqView for write_tile_group_obu without a parsed SequenceHeader; the ctor needs the CoreSeqInterView crate-private fields so it must live here (encoder-coreseq-inter-ctor / ENC-WRITER-INPUT-INTER-VIEW). +30 (2026-06-20): CoreSeqView::new_minimal_intra(max_w, max_h) -> Option<Self> — the minimal-intra seq-view encoder writer-input ctor (~120 lines incl. the §5.4.1 f(4) dim-bit-width validation and ceil_log2 derivation per the Codex P2 findings) + its parameterization/validation test, offset by removing the now-dead base_seq/test_sub_views/base_filter/base_restoration/base_ccso/base_inter helpers (~124); base_seq() now delegates to the ctor so the whole frame-header round-trip suite regresses it (encoder-coreseqview-ctor / ENC-WRITER-INPUT-SEQ-VIEW). TODO: the encoder writer-input ctors (CoreSeqInterView/CoreSeqView/future FrameHeaderCore) should move to a frame/encoder_input.rs submodule to stop growing this file",
    },
    SourceLineAllowance {
