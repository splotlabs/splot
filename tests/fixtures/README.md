...
to pass the current §5.4.1 parser.

| File | Bytes (hex) | What it is | `splot validate` |
|------|-------------|------------|------------------|
| `conformant.av2` | `01 08 05 04 82 0c cf dc` | TemporalDelimiter (len 1, hdr `08`) + minimal single-picture SequenceHeader (len 5, hdr `04` + §5.4.1 prefix payload) | conformant, exit `0` |
...
```bash
printf '\x01\x08\x05\x04\x82\x0c\xcf\xdc' > conformant.av2
