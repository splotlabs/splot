...
| `frame-header-core.av2` | `01 08 13 04 80 0c 01 77 0f 0f 00 00 00 05 70 00 00 06 00 10 00 02 06 10 f8 3c 3c 4c 80` | TemporalDelimiter + a non-single-picture SequenceHeader (id 0; `OrderHintBits = 1`, `NumRefFrames = 8`, 10-bit 4:2:0, 8-bit frame dimensions, 16×16 maximum) + an `OBU_CLOSED_LOOP_KEY` (len 6, hdr `10`) whose first tile group carries a frame header parsed through the full §5.18.2 intra structure cluster and the loop-filter cluster: a 16×16 output key frame (`immediate_output_frame = 1`, so the lone coded extended layer unit satisfies the §7.3.6 at-least-one-output-frame-unit rule) with a single-tile `tile_info()` (`uniform_tile_spacing_flag = 1`), `base_q_idx = 100` (`f(9)`, 10-bit), `segmentation_enabled = 0`, `using_qmatrix = 0`, `delta_q_present = 0`, then `deblocking_filter_params()` (`apply_deblocking_filter` all 0) with GDF/CDEF disabled at the sequence level, then `lr_params()` and `ccso_params()` (restoration and CCSO disabled at the sequence level, so both read no bits — `lr.uses_lr = false`, no `ccso_frame_flag`); `inspect --json` surfaces a `frame_header_core` summary (`status = stopped_before_read_tx_mode`, `frame_type = key`, `frame_size = 16×16`) | conformant, exit `0` |
| `frame-header-core-mfh.av2` | `01 08 13 04 80 0c 01 77 0f 0f 00 00 00 05 70 00 00 06 00 10 00 02 02 0c c2 05 10 a8 26 40 20` | `frame-header-core.av2`'s TemporalDelimiter + SequenceHeader (id 0), then an in-band `multi_frame_header_obu` (len 2, hdr `0c` = `obu_type` 3, payload `c2`) with `mfh_seq_header_id = 0`, `mfhId = 1`, `mfh_frame_size_present_flag = 0` (omitted → the §5.18.2 :4101 default-size inference applies), `mfh_deblocking_filter_update = 0`, and `mfh_seg_info_present_flag = 0`; then an `OBU_CLOSED_LOOP_KEY` (len 5, hdr `10`, payload `a8 26 40 20`) whose first tile group references `cur_mfh_id = 1`. The inspector resolves the MFH to its sequence header and parses the frame through the full §5.18.2 intra structure cluster and the loop-filter cluster: a 16×16 output key frame (`immediate_output_frame = 1`) whose §5.18.4.1 default dimensions come from the MFH (inferred to the 16×16 sequence maxima), `base_q_idx = 100`, segmentation/QM/delta-q disabled, then `deblocking_filter_params()` reading `apply_deblocking_filter[0]/[1]` from the frame (the MFH signalled no update; both 0) with GDF/CDEF disabled, then `lr_params()` and `ccso_params()` (restoration and CCSO disabled; both read no bits); `inspect --json` surfaces a `frame_header_core` summary (`cur_mfh_id = 1`, `frame_size = 16×16`, `status = stopped_before_read_tx_mode`) | conformant, exit `0` |
...
printf '\x01\x08\x13\x04\x80\x0c\x01\x77\x0f\x0f\x00\x00\x00\x05\x70\x00\x00\x06\x00\x10\x00\x02\x06\x10\xf8\x3c\x3c\x4c\x80' > frame-header-core.av2
