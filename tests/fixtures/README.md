...
| `frame-header-core.av2` | `01 08 13 04 80 0c 01 77 0f 0f 00 00 00 05 70 00 00 06 00 10 00 02 05 10 e8 3c 3c 00` | TemporalDelimiter + a non-single-picture SequenceHeader (id 0; `OrderHintBits = 1`, `NumRefFrames = 8`, 8-bit frame dimensions, 16×16 maximum) + an `OBU_CLOSED_LOOP_KEY` (len 5, hdr `10`) whose first tile group carries a frame header parsed through `frame_size()` to a 16×16 key frame; `inspect --json` surfaces a `frame_header_core` summary (`status = stopped_before_filtering_quant_segmentation`, `frame_type = key`, `frame_size = 16×16`) | conformant, exit `0` |
...
printf '\x01\x08\x13\x04\x80\x0c\x01\x77\x0f\x0f\x00\x00\x00\x05\x70\x00\x00\x06\x00\x10\x00\x02\x05\x10\xe8\x3c\x3c\x00' > frame-header-core.av2
