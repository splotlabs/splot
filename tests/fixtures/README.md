...
|------|-------------|------------|------------------|
| `conformant.av2` | `01 08 0b 04 82 0c cf dc 00 01 00 04 00 02` | TemporalDelimiter (len 1, hdr `08`) + a complete single-picture SequenceHeader (len 11, hdr `04` + general §5.4.1 fields, all §5.4 child configs, `film_grain_params_present`, and `trailing_bits`). The sequence header sits at `obu_xlayer_id = 0` with no frame-bearing OBU, so it is a header-only coded extended layer unit. Exercises the §5.4 sequence-header parse and `inspect` paths | `celu/missing-output-frame-unit` (§7.3.6 line 536: every CELU needs a coded output frame unit), exit `1` |
| `seq-header-tile-params.av2` | `01 08 0c 04 82 0c cf dc 00 01 00 04 00 14 80` | Same prefix but `seq_tile_info_present_flag = 1` with a uniform single-tile `tile_params()`, so `inspect --json` reports the sequence-header payload fully parsed (`AV2-5.4-SEQUENCE-HEADER`). Header-only CELU at `obu_xlayer_id = 0` (no frame-bearing OBU); used only by the `inspect` test | `celu/missing-output-frame-unit` (§7.3.6 line 536), exit `1` |
...
| `frame-header-core.av2` | `01 08 13 04 80 0c 01 77 0f 0f 00 00 00 05 70 00 00 06 00 10 00 02 08 10 f8 3c 3c 4c 80 00 00` | TemporalDelimiter + a non-single-picture SequenceHeader (id 0; `OrderHintBits = 1`, `NumRefFrames = 8`, 10-bit 4:2:0, 8-bit frame dimensions, 16×16 maximum, `film_grain_params_present = 0`) + an `OBU_CLOSED_LOOP_KEY` (len 8, hdr `10`) whose first tile group carries a frame header parsed through the **complete** §5.18.2 intra path: the structure cluster and loop-filter cluster — a 16×16 output key frame (`immediate_output_frame = 1`, so the lone coded extended layer unit satisfies the §7.3.6 at-least-one-output-frame-unit rule) with a single-tile `tile_info()` (`uniform_tile_spacing_flag = 1`), `base_q_idx = 100` (`f(9)`, 10-bit), `segmentation_enabled = 0`, `using_qmatrix = 0`, `delta_q_present = 0`, `deblocking_filter_params()` (`apply_deblocking_filter` all 0) with GDF/CDEF disabled, `lr_params()` and `ccso_params()` (restoration and CCSO disabled, so both read no bits — `lr.uses_lr = false`, no `ccso_frame_flag`) — then the §5.18.2 tail `read_tx_mode()` (`tx_mode_select = 0` → `TX_MODE_LARGEST`), the no-bit intra inferences, `reduced_tx_set = 0`, the no-bit `global_motion_params()` intra arm, and `film_grain_config()` (grain absent → `apply_grain = 0`). The frame header ends mid-byte 5, so the §5.19 `tile_group_obu()` `byte_alignment()` (NumTiles == 1 → no `tile_start_and_end_present_flag`; `tg_start = 0`, `tg_end = 0` inferred) zero-pads the rest of the final `00` byte (the §6.2.4 `zero_bit`s are 0) and `tile_group_payload(sz)` carries one coded-tile byte (`headerBytes == 6`, `sz == 7`, so the single last tile takes `tileSize = 1` — a zero-size non-bridge tile would start `SymbolMaxBits` at `-15`, below the §8.2.4 `exit_symbol()` floor of `-14`, see `tile-payload/zero-size-tile`). `inspect --json` surfaces a `frame_header_core` summary (`status = intra_header_complete`, `frame_type = key`, `frame_size = 16×16`, `intra_tail.tx_mode = tx_mode_largest`) plus a `tile_group_structure` view (`num_tiles = 1`, `tg_start = 0`, `tg_end = 0`, `status = complete`, `payload_size = 1`) | conformant, exit `0` |
| `frame-header-core-mfh.av2` | `01 08 13 04 80 0c 01 77 0f 0f 00 00 00 05 70 00 00 06 00 10 00 02 02 0c c2 06 10 a8 26 40 20 00` | `frame-header-core.av2`'s TemporalDelimiter + SequenceHeader (id 0), then an in-band `multi_frame_header_obu` (len 2, hdr `0c` = `obu_type` 3, payload `c2`) with `mfh_seq_header_id = 0`, `mfhId = 1`, `mfh_frame_size_present_flag = 0` (omitted → the §5.18.2 :4101 default-size inference applies), `mfh_deblocking_filter_update = 0`, and `mfh_seg_info_present_flag = 0`; then an `OBU_CLOSED_LOOP_KEY` (len 6, hdr `10`, payload `a8 26 40 20 00`) whose first tile group references `cur_mfh_id = 1`. The inspector resolves the MFH to its sequence header and parses the frame through the **complete** §5.18.2 intra path: a 16×16 output key frame (`immediate_output_frame = 1`) whose §5.18.4.1 default dimensions come from the MFH (inferred to the 16×16 sequence maxima), `base_q_idx = 100`, segmentation/QM/delta-q disabled, then `deblocking_filter_params()` reading `apply_deblocking_filter[0]/[1]` from the frame (the MFH signalled no update; both 0) with GDF/CDEF disabled, `lr_params()` and `ccso_params()` (restoration and CCSO disabled; both read no bits), then the §5.18.2 tail (`read_tx_mode()` `tx_mode_select = 0`, `reduced_tx_set = 2`, grain absent → `apply_grain = 0`), consuming the padding so the parse completes; `inspect --json` surfaces a `frame_header_core` summary (`cur_mfh_id = 1`, `frame_size = 16×16`, `status = intra_header_complete`) | conformant, exit `0` |
...
| `quantizer-matrix.av2` | `01 08 0b 04 82 0c cf dc 00 01 00 04 00 02 04 58 00 02 c0` | TemporalDelimiter + the `conformant.av2` sequence header + a `quantizer_matrix_obu` (len 4, hdr `58` = `obu_type` 22) with `qm_bit_map = 1` (level 0), `qm_chroma_info_present_flag = 0` (1 plane), and `qm_is_default_flag = 1` (default matrix); `inspect --json` surfaces a `quantizer_matrix` view. Header-only CELU at `obu_xlayer_id = 0` (no frame-bearing OBU) | `celu/missing-output-frame-unit` (§7.3.6 line 536), exit `1` |
| `film-grain.av2` | `01 08 0b 04 82 0c cf dc 00 01 00 04 00 02 06 5c 01 80 00 00 40` | TemporalDelimiter + the `conformant.av2` sequence header + a `film_grain_obu` (len 6, hdr `5c` = `obu_type` 23) with `fgm_update_flags = 1` (slot 0), `fgm_chroma_idc = 0` (4:2:0), and one minimal `film_grain_model` (no scaling points, `ar_coeff_lag = 0`); `inspect --json` surfaces a `film_grain` view. Header-only CELU at `obu_xlayer_id = 0` (no frame-bearing OBU) | `celu/missing-output-frame-unit` (§7.3.6 line 536), exit `1` |
...
printf '\x01\x08\x13\x04\x80\x0c\x01\x77\x0f\x0f\x00\x00\x00\x05\x70\x00\x00\x06\x00\x10\x00\x02\x08\x10\xf8\x3c\x3c\x4c\x80\x00\x00' > frame-header-core.av2
printf '\x01\x08\x13\x04\x80\x0c\x01\x77\x0f\x0f\x00\x00\x00\x05\x70\x00\x00\x06\x00\x10\x00\x02\x02\x0c\xc2\x06\x10\xa8\x26\x40\x20\x00' > frame-header-core-mfh.av2
