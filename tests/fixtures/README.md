...
| `frame-header-core.av2` | `01 08 13 04 80 0c 01 77 0f 0f 00 00 00 05 70 00 00 06 00 10 00 02 07 10 f8 3c 3c 4c 80 10` | TemporalDelimiter + a non-single-picture SequenceHeader (id 0; `OrderHintBits = 1`, `NumRefFrames = 8`, 10-bit 4:2:0, 8-bit frame dimensions, 16×16 maximum, `film_grain_params_present = 0`) + an `OBU_CLOSED_LOOP_KEY` (len 7, hdr `10`) whose first tile group carries a frame header parsed through the **complete** §5.18.2 intra path: the structure cluster and loop-filter cluster — a 16×16 output key frame (`immediate_output_frame = 1`, so the lone coded extended layer unit satisfies the §7.3.6 at-least-one-output-frame-unit rule) with a single-tile `tile_info()` (`uniform_tile_spacing_flag = 1`), `base_q_idx = 100` (`f(9)`, 10-bit), `segmentation_enabled = 0`, `using_qmatrix = 0`, `delta_q_present = 0`, `deblocking_filter_params()` (`apply_deblocking_filter` all 0) with GDF/CDEF disabled, `lr_params()` and `ccso_params()` (restoration and CCSO disabled, so both read no bits — `lr.uses_lr = false`, no `ccso_frame_flag`) — then the §5.18.2 tail `read_tx_mode()` (`tx_mode_select = 0` → `TX_MODE_LARGEST`), the no-bit intra inferences, `reduced_tx_set = 0`, the no-bit `global_motion_params()` intra arm, and `film_grain_config()` (grain absent → `apply_grain = 0`). The final `10` byte is the new tail + `trailing_bits()`. `inspect --json` surfaces a `frame_header_core` summary (`status = intra_header_complete`, `frame_type = key`, `frame_size = 16×16`, `intra_tail.tx_mode = tx_mode_largest`) | conformant, exit `0` |
...
printf '\x01\x08\x13\x04\x80\x0c\x01\x77\x0f\x0f\x00\x00\x00\x05\x70\x00\x00\x06\x00\x10\x00\x02\x07\x10\xf8\x3c\x3c\x4c\x80\x10' > frame-header-core.av2
