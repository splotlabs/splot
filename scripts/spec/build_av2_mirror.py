...
def render_provenance(meta: dict[str, str]) -> str:
    return f"""# Provenance of the committed AV2 v1.0.0 specification mirror.
...
def main() -> int:
    ...
    provenance_toml = render_provenance(meta)
    ...
    checksum_lines = [
        f"{sha256_text(files[rel])}  {rel}" for rel in sorted(files)
    ]
