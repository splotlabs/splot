# ac0ej3 Luma TxType Residual Handoff Specification

## Purpose
Define the fail-closed ac0ej3 Wiener NS LR handoff that retains resolved luma
transform types for syntax-only residual parsing while decoded sample
reconstruction remains unsupported.

## Requirements

### Requirement: ac0ej3 luma transform-type residual handoff

The decoder SHALL track `DECODE-AC0EJ3-LUMA-TXTYPE-RESIDUAL-HANDOFF` as a
partial runtime prerequisite for the ac0ej3 Wiener NS LR path. When selectable
...
- **WHEN** the local ac0ej3 Wiener NS LR transform-record path reaches nonzero
