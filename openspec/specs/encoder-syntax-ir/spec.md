...
#### Scenario: Out-of-order children are rejected before mutation

- **WHEN** a plan constructor receives child decisions whose explicit indices
  are not strictly ordered where ordering is required
...
The encoder SHALL validate planning dimensions, counts, coefficient entries,
and event counts through bounded constructors before a future writer can consume
the plan.

#### Scenario: Invalid coefficient plans fail

- **WHEN** a quantized coefficient plan contains duplicate coefficient indices
  or zero-valued coefficient entries
- **THEN** construction fails with a typed planning error
