# local decoder mission Selectable Transform Records Specification

## Purpose
Define the fail-closed local decoder mission Wiener NS loop-restoration frontier that parses
...
### Requirement: local decoder mission Selectable Transform Records

The decoder SHALL track `DECODE-SELECTABLE-TRANSFORM-RECORDS` as a
partial runtime prerequisite for the local decoder mission Wiener NS LR path. For supported
...
local decoder mission stream. When the local stream reaches active CfL chroma mode syntax while
deriving those records, the active CfL prerequisite SHALL be tracked by
`DECODE-CFL-CHROMA-MODE-FRONTIER` and SHALL not be counted as completed
...
be tracked by
`DECODE-SELECTABLE-NARROW-LUMA-RECORDS` until the observed luma-only
...
that prerequisite SHALL be tracked by `DECODE-SDP-CFL-ALLOWED-FRONTIER`
...
SHALL be tracked by `DECODE-INTRA-PRELUDE-TX-FRONTIER` until the
...
- **WHEN** the local decoder mission stream reaches active transform-record
...
### Requirement: local decoder mission luma-only narrow selectable record handoff

The decoder SHALL track `DECODE-SELECTABLE-TRANSFORM-RECORDS` as the
owner of the bounded luma-only narrow selectable-record handoff. When the local
local decoder mission Wiener NS LR path reaches a supported luma-only narrow selectable leaf,
...
- **WHEN** the local decoder mission stream reaches a luma-only narrow selectable
  transform leaf
...
- **THEN** the decoder SHALL NOT claim decoded `CurrFrame` or `CdefFrame`
  ...
  reference refresh, AVM/dav2d byte equality, or successful local decoder mission decode
