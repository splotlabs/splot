# local decoder mission Active Intra IST Handoff Specification

## Purpose

Track the local decoder mission decoder mission boundary where active intra IST syntax is
consumed for Wiener NS LR tx-skip record derivation without claiming secondary
inverse-transform reconstruction.

## Requirements

### Requirement: local decoder mission Active Intra IST Handoff

The decoder SHALL track `DECODE-ACTIVE-INTRA-IST-HANDOFF` as a partial
runtime prerequisite for the local decoder mission decode mission. For the local decoder mission Wiener
...
or successful local decoder mission decode.

#### Scenario: Active IST syntax is handed to LR tx-skip records

- **WHEN** the local decoder mission stream reaches Wiener NS LR transform-record
  derivation
...
- **THEN** `DECODE-ACTIVE-INTRA-IST-HANDOFF` remains partial
