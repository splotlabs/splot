# local decoder mission SDP CflAllowedInSdp Frontier Specification

## Purpose
Define the fail-closed local decoder mission Wiener NS loop-restoration frontier that retains
...
### Requirement: local decoder mission SDP CflAllowedInSdp Frontier

The decoder SHALL track `DECODE-SDP-CFL-ALLOWED-FRONTIER` as a partial
runtime prerequisite for the local decoder mission Wiener NS LR path. During intra SDP
...
#### Scenario: Local local decoder mission advances past the uv-mode desync gate

- **WHEN** the local decoder mission stream reaches the current SDP chroma
...
### Requirement: local decoder mission Intra Prelude Transform Frontier

The decoder SHALL track `DECODE-INTRA-PRELUDE-TX-FRONTIER` as a partial
runtime prerequisite for the local decoder mission Wiener NS LR selectable transform-record
...
- **WHEN** an local decoder mission luma/shared intra leaf has coded `use_intrabc`, CDEF, and
