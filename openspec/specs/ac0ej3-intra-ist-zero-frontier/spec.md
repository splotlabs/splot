# ac0ej3-intra-ist-zero-frontier Specification

## Purpose

Track the ac0ej3 decoder mission boundary where intra IST secondary-transform
syntax is read for the covered DCT_DCT luma residual subset, while active
secondary transforms remain unsupported.

## Requirements

### Requirement: ac0ej3 Intra IST Zero Frontier

The decoder SHALL track `DECODE-AC0EJ3-INTRA-IST-ZERO-FRONTIER` as a partial
runtime prerequisite for the ac0ej3 decode mission. For the covered intra luma
...
- **WHEN** the local ac0ej3 residual path reaches an intra luma DCT_DCT block
...
- **THEN** `DECODE-AC0EJ3-INTRA-IST-ZERO-FRONTIER` remains partial
- **AND** the decoder does not claim successful ac0ej3 output, raw/Y4M output,
