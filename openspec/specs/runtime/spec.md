...
### Requirement: Minimal runtime reports local decoder mission Wiener NS frontier precisely
...
diagnostic SHALL track `DECODE-WIENERNS-FRONTIER`, cite AV2 5.18.7.11,
...
#### Scenario: local decoder mission reaches Wiener NS frontier

- **WHEN** `splot decode` runs against the local decoder mission fixture selected
  via `SPLOT_LOCAL_DECODER_MISSION_IVF` or the documented `$HOME/Documents/SplotLabs/local-decoder-mission.ivf`
  fallback with default decode limits
- **THEN** it rejects before output with `unsupported_reason =
  "unsupported_wienerns_filter"`
- **AND** the diagnostic tracks `DECODE-WIENERNS-FRONTIER`
