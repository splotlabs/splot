...
Track the local decoder mission fail-closed runtime prerequisite for retaining value-backed
`LrTxSkip` grid values before live tile traversal wires transform records into
loop-restoration classification.

## Requirements

### Requirement: local decoder mission LR Tx-Skip Grid Retention

The decoder SHALL track `DECODE-LR-TX-SKIP-GRID-RETENTION` as a partial
local decoder mission Wiener NS LR prerequisite. The helper SHALL derive luma `LrTxSkip`
...
#### Scenario: Live local decoder mission remains fail-closed

- **WHEN** the local decoder mission stream reaches the live LR runtime storage
  retention boundary
