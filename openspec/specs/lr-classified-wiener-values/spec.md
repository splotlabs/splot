...
Track the local decoder mission fail-closed runtime frontier for AV2 §7.20.4 classified-Wiener
value derivation.

## Requirements

### Requirement: local decoder mission Classified Wiener Value Frontier

The decoder SHALL track `DECODE-LR-CLASSIFIED-WIENER-VALUES` as a
...
#### Scenario: Live local decoder mission stops at storage boundary
...
- **THEN** the runtime SHALL return `decode/unsupported-feature` referencing
  `DECODE-LR-CLASSIFIED-WIENER-VALUES`
- **AND** the diagnostic SHALL NOT claim real source sample reads, real
  `LrTxSkip` reads, loop-restoration filtering, output, or successful local decoder mission
  decode
