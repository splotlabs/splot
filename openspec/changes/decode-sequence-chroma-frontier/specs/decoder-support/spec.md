...
### Requirement: Decoder support tracks local decoder mission sequence chroma frontier

The decoder support model SHALL include a partial row for
`DECODE-SEQUENCE-CHROMA-FRONTIER` named
`sequence-chroma-frontier`. The row SHALL describe that the runtime parses
...
- **WHEN** `cargo xtask check-decoder-support` validates the decoder support
  metadata
- **THEN** the `sequence-chroma-frontier` row exists with Feature ID
  `DECODE-SEQUENCE-CHROMA-FRONTIER`
- **AND** the row records the focused runtime and local `local decoder mission` regression tests
...
- **THEN** CfL prediction, MHCCP prediction, `read_cfl_alphas`, 10-bit
  reconstruction/output, key-frame loop filters, and full `local decoder mission` decode remain
