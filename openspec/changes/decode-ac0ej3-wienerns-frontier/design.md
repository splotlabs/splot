...
- Keep the local ac0ej3 regression pinned to byte offset 74 and to the precise
...
- Keep all existing non-Wiener incomplete-header statuses on the old generic
  fallback.
  Rationale: only the Wiener NS stop is proven by the ac0ej3 mission stream in
  this brick.
...
- Risk: old rows still mention the former live gate.
  Mitigation: update adjacent ac0ej3 and minimal-tier notes so generated status
