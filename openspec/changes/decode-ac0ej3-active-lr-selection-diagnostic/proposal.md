...
aggregate activity row. That makes the current ac0ej3 gate look older than the
...
- `decoder-support`: the live ac0ej3 active LR-unit diagnostic is owned by the
  selection-state frontier after per-unit state is retained.

## Impact

Affected areas: `crates/splot-decode`, local ac0ej3 CLI regression, decoder
