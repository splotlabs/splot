...
Before this change, the live local decoder mission key frame reported the exact frame-level
Wiener NS frontier, but the core parser still stopped before consuming any
`read_wienerns_filter()` syntax. The local decoder mission prefix proves a narrow luma
...
- Add Feature ID `DECODE-WIENERNS-BANK-FRONTIER` for the next local decoder mission
  loop-restoration parser advance.
- Parse the AV2 5.20.10.6 frame-level `read_wienerns_filter(0, 0, 0, 1)` fixed
  syntax for the intra/luma shape reached by local decoder mission, including fixed-coded match
  prediction, preserving the parsed coefficients in the core model.
- Let the local decoder mission key frame advance from `stopped_before_wienerns_filter` to a
...
- Keep loop-restoration reconstruction, LR unit syntax, inter temporal-copy
  ...
  local decoder mission decode out of scope.
...
- `runtime`: the minimal runtime SHALL continue rejecting local decoder mission before output,
...
- `decoder-support`: decoder support metadata SHALL track the narrow local decoder mission
...
- Tests: core parser unit tests for the fixed-coded luma frame-filter bank,
  local decoder mission CLI regression, and focused runtime checks.
