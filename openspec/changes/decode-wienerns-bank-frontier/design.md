...
`frame_filters_on`. The live local decoder mission key frame reaches that stop with luma
...
- Parse and model the proven intra luma frame-filter-bank syntax for
...
- Advance the live local decoder mission diagnostic past `unsupported_wienerns_filter` to the
...
- No entropy-coded LR unit bank update for tile syntax, 10-bit output, or
  successful local decoder mission decode. The fixed-coded frame-filter parser may translate
...
- Model only the intra frame-level path in this brick.
  Rationale: on the local decoder mission key frame, `NumTotalRefs == 0`, so
