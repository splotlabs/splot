...
The local ac0ej3 probe parses every general-intra block of frame 0 — modes,
selectable transform records, coefficients, IntrABC vectors — through the
`DECODE-AC0EJ3-SELECTABLE-TRANSFORM-RECORDS` walk, then fails closed without ever
...
missing bridge for Milestone 1 (a bit-exact ac0ej3 key frame): the parse path and
...
- Add a reconstruction sink that owns a `CurrentFrameWorkspace<u16>` sized to the
  ac0ej3 frame and reconstructs each NON-IntrABC general-intra luma transform
...
- The public `splot decode ac0ej3` path STAYS FAIL-CLOSED: the selectable walk
...
`68`) against the AVM pre-filter reconstruction oracle — the first bit-exact ac0ej3
reconstruction milestone, proven by
`ac0ej3_frame_origin_dc_block_reconstructs_bit_exact_against_prefilter_oracle`. The
...
- `ac0ej3-selectable-transform-records`: add a reconstruction bridge that
  reconstructs the verified NON-IntrABC general-intra DC region of ac0ej3 frame 0
  ...
  bit-exact vs the AVM pre-filter oracle (first bit-exact ac0ej3 reconstruction
...
- `decoder-support`: update support-row requirements and proof expectations for
  the ac0ej3 selectable-transform frontier after the reconstruction bridge.
...
- Non-goals: IntrABC reconstruction, non-DC intra modes, deblock/CDEF/LR over the
  ac0ej3 frame, decoded-frame output for the public path, reference refresh,
  AVM/dav2d byte equality for the whole frame, and successful ac0ej3 decode.
