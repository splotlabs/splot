...
`local-decoder-mission.ivf` is profile 0 10-bit 4:2:0, while the current minimal runtime only
...
- Pin the leading payload ordering so the local mission regression parses the
  ...
  `decode-10bit-sequence-frontier` and
  `decode-sequence-chroma-frontier` changes move parsing to the key-frame
  header, and `decode-wienerns-frontier` now reports the live gate as
