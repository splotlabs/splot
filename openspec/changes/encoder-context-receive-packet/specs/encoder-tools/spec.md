...
visible Y/U/V sample equals the 128 AV2 § 7.13.2 no-neighbour DC predictor — by emitting the
minimal skip frame, tracked by `ENC-CONTEXT-RECEIVE-PACKET`. Because such a frame has zero
...
- **WHEN** an all-128 64x64 YUV420 frame is sent to a `Context` and `receive_packet` is called
  after `flush`
- **THEN** `receive_packet` SHALL return `Packet` carrying non-empty coded bytes
- **AND** decoding those bytes with `splot-decode` SHALL reconstruct a 6144-byte frame whose every
  sample equals the all-128 input (`decode(encode(input)) == input`)
