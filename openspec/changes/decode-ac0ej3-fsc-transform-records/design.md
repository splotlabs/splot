...
The ac0ej3 Wiener NS LR runtime path now parses the local stream far enough to
...
frame-facts `useFsc` wrapper. The missing piece is the ac0ej3 LR record path
...
- Consume the observed ac0ej3 luma `fsc_mode` syntax in the Wiener NS LR
...
- Retain `skip_flag`/`eob` facts needed for live `LrTxSkip` storage and advance
  the local ac0ej3 unsupported diagnostic frontier.
- Keep unobserved FSC shapes, chroma FSC, decoded samples, loop restoration,
  output, and reference refresh fail-closed.

**Non-Goals:**

- Broad FSC/IDTX support outside the local ac0ej3 LR record walk.
...
2. Treat `fsc_mode` as a record-walk fact, not reconstruction support.
   ...
   Rationale: the current ac0ej3 path only needs syntax consumption and
...
3. Keep the supported subset local and explicit.
   ...
   luma transform-record subcase in the local ac0ej3 stream; other plane,
...
- [Risk] Advancing the unsupported frontier may expose a larger downstream
  ...
  Mitigation: keep the CLI ignored ac0ej3 test asserting the precise current
