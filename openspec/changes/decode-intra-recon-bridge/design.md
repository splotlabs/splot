# Design: local decoder mission general-intra reconstruction bridge
...
2. `wienerns_lr/tx_records.rs` is the local decoder mission path. The selectable-transform
...
## local decoder mission frame-0 facts (AVM inspect)
...
- Oracle: `local_prefiltered_oracle.yuv` (intra + residual, BEFORE deblock/CDEF/LR), md5
...
Owns:
- `workspace: CurrentFrameWorkspace<u16>` sized to the local decoder mission frame (via the
...
A new test:
1. Decodes the local decoder mission fixture with a sink attached, driving the selectable
...
3. Asserts that region equals a committed assertion: a SHA-256 of the verified
   first-SB sample region (derived offline from `local_prefiltered_oracle.yuv`), plus a
   spot check that the top-left 8x8 luma is all `68`.

The test is gated to the local mission fixture (`SPLOT_LOCAL_DECODER_MISSION_IVF` / `#[ignore]`),
matching the existing `local_decoder_mission_*` probe convention. The committed assertion
...
desync: local decoder mission's first superblock now parses bit-exact vs the AVM analyzer.

Fed the now-correct parse, the bridge reconstructs the verified DC subset to the
spec-correct samples. Verified against the `local_prefiltered_oracle.yuv` oracle:

- The frame-origin §5.20.5.3 `DC_PRED` 16x16 luma leaf reconstructs BIT-EXACT (all
  `68`) — the first bit-exact local decoder mission reconstruction milestone.
