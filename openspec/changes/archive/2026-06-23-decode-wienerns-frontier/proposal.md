...
The live local decoder mission decode now advances past the sequence chroma-tool gate, but the
...
- Add Feature ID `DECODE-WIENERNS-FRONTIER` for the local decoder mission Wiener NS
...
- Update the local decoder mission CLI regression and decoder tracking docs to expect
...
- `runtime`: the minimal runtime SHALL surface the local decoder mission Wiener NS parser stop as
  a precise unsupported-feature diagnostic instead of the generic incomplete
  header fallback.
- `decoder-support`: decoder support metadata SHALL track the local decoder mission Wiener NS
...
- Tests: focused runtime diagnostic tests plus the ignored local decoder mission CLI
  regression.
