...
The local ac0ej3 probe now reaches the selectable transform-record walk and
...
- Extend the ac0ej3 selectable-transform record path for
  `DECODE-AC0EJ3-SELECTABLE-TRANSFORM-RECORDS` to recognize the observed
...
- `ac0ej3-selectable-transform-records`: add a bounded IntrABC mode-info record
  handoff for the local ac0ej3 Wiener NS LR selectable transform-record path.
- `decoder-support`: update support-row requirements and proof expectations for
  the ac0ej3 selectable-transform frontier after IntrABC syntax consumption.
