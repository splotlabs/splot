...
PR #487 moves the local ac0ej3 probe past the active FSC residual handoff and
...
- Consume the observed `use_intrabc = 1` mode-info path in spec order for the
  ac0ej3 selectable-transform record handoff.
...
- Broad IntrABC support outside the local ac0ej3 path.
...
4. Keep the old zero-IntrABC behavior as a regression boundary.
   ...
   Existing selectable-transform tests and the previous ac0ej3 frontier prove
...
- Wrong §5.20.5.4 read order could satisfy bit-count checks while corrupting
  ...
  IntrABC helper and verify with the local ignored ac0ej3 probe.
