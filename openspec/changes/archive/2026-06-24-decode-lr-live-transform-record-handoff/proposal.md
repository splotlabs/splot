...
The local decoder mission decoder mission currently has the pieces to derive a retained
...
Feature ID: `DECODE-LR-LIVE-TRANSFORM-RECORD-HANDOFF`.
...
- Move the local decoder mission stream from the allocation-only diagnostic to a
...
- Keep decoded `CurrFrame`/`CdefFrame` samples, `FilterClass`,
  ...
  local decoder mission decode explicitly out of scope.

## Capabilities

### New Capabilities

- `lr-live-transform-record-handoff`: Tracks the live runtime handoff
  from parsed luma tile transform facts to live LR `LrTxSkip` storage, plus the
  local decoder mission fail-closed selectable-transform frontier.
...
- Validator diagnostics remain structured and fail-closed; the local decoder mission
