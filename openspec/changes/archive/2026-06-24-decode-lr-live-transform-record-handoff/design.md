...
The local decoder mission key frame uses `TX_MODE_SELECT`, so its transform records also
...
- Move the local decoder mission fixture to a precise `TX_MODE_SELECT` transform-record
...
- Implement `FilterClass`, `SubclassLookup`, loop-restoration filtering/output,
  reference refresh, or full local decoder mission oracle parity.
...
- Fixed-largest support does not advance the local decoder mission fixture past transform
  records because local decoder mission uses selectable transforms. Mitigation: the new
...
- Reading transform records from tile facts without reconstruction can drift from
  ...
  population, and local decoder mission diagnostic movement.
