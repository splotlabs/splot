...
- [x] 1.3 Delegate `tokenize_coefficients` to `luma_dc_coded_tokens` and raise the supported magnitude cap to `LF_NUM_BASE_LEVELS + COEFF_BASE_RANGE + 1 = 8`.
...
- [x] 2.2 Extend `coded_dc_tokens_match_tokenizer` to the full 1..=8 range; add base-range token-shape and roundtrip tests.
