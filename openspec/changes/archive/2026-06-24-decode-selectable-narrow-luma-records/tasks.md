...
- [x] 2.1 Verify the local decoder mission probe advances past `unsupported_wienerns_lr_selectable_transform_records_block_shape`.
...
- [x] 3.1 Add `DECODE-SELECTABLE-NARROW-LUMA-RECORDS` to the implementation and decoder-support matrices, update spec mapping if needed, and regenerate generated status docs.
- [x] 3.2 Run focused tests, the local decoder mission decode probe, `openspec validate --all --no-interactive`, `cargo xtask feature-status`, `cargo xtask check-feature-status`, and `cargo xtask ci`.
