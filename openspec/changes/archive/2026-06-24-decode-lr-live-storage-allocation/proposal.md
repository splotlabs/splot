...
The live local decoder mission path now derives the 10-bit `CurrFrame`/`CdefFrame` storage
...
- Add Feature ID `DECODE-LR-LIVE-STORAGE-ALLOCATION` and matrix row
...
- Move the live local decoder mission unsupported diagnostic to the next honest boundary:
...
### New Capabilities
- `lr-live-storage-allocation`: specific fail-closed runtime
  allocation for unpopulated 10-bit loop-restoration frame buffers and
  `LrTxSkip` grid storage.

### Modified Capabilities
- `decoder-support`: records that the live local decoder mission diagnostic advances from
  runtime storage-retention planning to explicit live storage allocation.
- `lr-runtime-storage-retention`: records that this row remains the
  storage-footprint planning contract while the live local decoder mission diagnostic is
...
- Affected code: `crates/splot-decode/src/runtime_minimal.rs`,
  ...
  tests, the ignored local decoder mission CLI gate test, and decoder
  support/feature-status docs.
- Diagnostics: the live local decoder mission `decode/unsupported-feature` reason advances
