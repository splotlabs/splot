...
- Keep local ac0ej3 decode fail-closed and update support tracking.
...
4. Treat transform-tool sequence flags as active-use gates, not sequence gates.
   ...
   For all-zero transform blocks, §5.20.7.27 does not reach `transform_type()` or CCTX. For nonzero blocks under the enabled ac0ej3 transform-tool set, the decoder will reject before continuing into coefficient parsing because the current helper does not consume transform-type/CCTX/IST syntax.
