...
The current local decoder mission frontier is `unsupported_wienerns_filter_bank`: the core
...
- Track the work as `DECODE-LR-UNIT-SYNTAX-FRONTIER`.
...
- Preserve fail-closed behavior before reconstruction, output, reference
  retention, and successful local decoder mission decode.
...
- Consume only `FrameRestorationType::WienerNonsep` planes with
  `frame_filters_on == true`. This matches the local decoder mission frame-level bank path and
