...
`DECODE-LR-SOURCE-BOUNDS-FRONTIER` retained active AV2 §7.20.1
source-bound facts for proven frame-level Wiener NS loop-restoration
...
- Add a distinct `DECODE-LR-SOURCE-READ-FRONTIER` matrix/support row.
...
- Preserve fail-closed behavior: the local decoder mission fixture must still stop before
...
- PC-Wiener classification, switchable LR, temporal/reference Wiener state,
  GDF/CDEF/deblock orchestration, 10-bit output, or successful local decoder mission decode.
...
- Keep the runtime diagnostic as the product of this brick. Non-classified paths
  should prove the source-read boundary was reached, while the local decoder mission
  two-class luma path should stop at the classified-Wiener boundary before the
  later filtering surface.
- Do not fabricate frame-buffer reads. Because the current local decoder mission path cannot yet
...
- Source reads need frame buffers that the current local decoder mission path may not allocate
