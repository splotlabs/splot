...
The local decoder mission runtime now consumes Wiener NS LR-unit selections, but the live
...
- Add Feature ID `DECODE-LR-SOURCE-BOUNDS-FRONTIER` for the narrow
...
- Add the Wiener NS length, UV-symmetry, and base CDF rows needed to consume the
  §5.20.10.6 entropy-coded per-unit filter syntax reached by local decoder mission chroma
  planes; do not expose or apply the decoded coefficients.
- Move the local decoder mission runtime diagnostic from the active unit-selection gate
...
- `decoder-support`: track the local decoder mission LR source-bounds frontier without
