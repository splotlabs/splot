...
The local decoder mission LR path has progressed to a live fail-closed storage-retention gate:
...
- Add `DECODE-LR-TX-SKIP-GRID-RETENTION` as a partial local decoder mission LR feature.
...
- Keep the live local decoder mission diagnostic fail-closed before decoded sample population,
  `FilterClass` grid retention, LR filtering, output, and reference refresh.

**Non-Goals:**

- No broad parse-only traversal of the full local decoder mission 1920x1080 key tile.
...
- [Risk] A helper-only brick can look more complete than the live runtime.
  ...
  parsing still does not populate the grid and local decoder mission remains fail-closed.
