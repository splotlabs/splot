# Change: Decode local decoder mission IntrABC NEWMV records

## Summary

Advance the local `local-decoder-mission.ivf` Wiener NS LR selectable-transform runtime frontier
...
- In the local decoder mission IntrABC path, derive the bounded reference block-vector
...
- Loop-restoration filtering, reference refresh, final output, or successful
  full `local-decoder-mission.ivf` decode.
