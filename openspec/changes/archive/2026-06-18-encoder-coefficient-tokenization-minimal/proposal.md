...
- Add a minimal coefficient-tokenization module for the existing 4x4 DCT_DCT
  DC-only quantized-coefficient subset.
- Derive scan order, EOB, begin-position metadata, sign/magnitude token facts,
  and ordered entropy-token records for AV2 §5.20.7.27 / §5.20.7.28.
...
- `encoder-tools`: add a requirement for minimal private coefficient
  tokenization over the current 4x4 DCT_DCT DC-only encoder subset.
