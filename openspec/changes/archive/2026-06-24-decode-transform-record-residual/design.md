...
The current live `local-decoder-mission.ivf` probe reaches the active Wiener NS LR
...
```json
{
  ...
  "matrix_row": "selectable-transform-records"
}
```
...
- Advance the local decoder mission probe to the next structured unsupported frontier and
...
4. **Test the observed geometry/order fact, not only the live file.** The local
   `local-decoder-mission.ivf` probe is useful evidence, but committed tests must be
