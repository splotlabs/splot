...
The previous ac0ej3 slice advanced the live Wiener NS LR path into selectable
...
- Consume active `is_cfl` mode-info and `read_cfl_alphas()` in AV2 spec order
  for the non-lossless 4:2:0 ac0ej3 path.
...
- Replace the live ac0ej3 diagnostic with the next honest unsupported frontier.
...
- Implement CfL prediction, `CflRef` capture, alpha application, chroma sample
  ...
  refresh.
- Claim successful ac0ej3 decode or AVM/dav2d byte equality.
...
- Wrong alpha-context selection could desynchronize the stream -> mitigate with
  focused unit tests for `cfl_alpha_signs` contexts and the live ac0ej3 probe.
