...
- [x] 1.2 Add implementation-matrix and decoder-support rows for `DECODE-AC0EJ3-SELECTABLE-TRANSFORM-RECORDS`.
- [x] 1.3 Update the existing LR handoff row notes so they point at the selectable transform-record follow-on instead of claiming the local ac0ej3 stream still stops there.
...
- [x] 2.2 Parse supported AV2 §5.20.6.1/§5.20.6.3 `TX_MODE_SELECT` transform records for the ac0ej3 intra LR path.
...
- [x] 3.2 Move the local ac0ej3 CLI regression past `unsupported_wienerns_lr_tx_mode_select_transform_records` to the next decoded-sample prerequisite.
