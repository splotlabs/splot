...
The local decoder mission probe now reaches AV2 §5.20.7.27 coefficient syntax at byte
...
- Add Feature ID `DECODE-DCTONLY-RESIDUAL-FRONTIER`.
- Teach the local decoder mission Wiener NS LR selectable transform-record path to distinguish
...
- `dctonly-residual-frontier`: DCT-only residual admission for the local
  local decoder mission Wiener NS LR transform-record frontier.
...
- Updates implementation and decoder support matrices plus generated status
  documents.
- Adds focused tests and refreshes the local decoder mission probe expectation.
