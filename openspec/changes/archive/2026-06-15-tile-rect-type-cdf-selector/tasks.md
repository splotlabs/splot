...
- [x] 1.1 Validate the OpenSpec proposal, design, spec delta, and agent log for `tile-rect-type-cdf-selector`.
- [x] 1.2 Record planning, implementation, testing, documentation, and review-agent findings in `agent-log.md`.

## 2. Tile Rect Type CDF Selector Expansion

- [x] 2.1 Extend `TileCdfRows` to own `RectTypeCdf` rows copied from generated `splot-core` defaults.
- [x] 2.2 Add typed selector and error-reporting support for `TileRectTypeCdf`.
- [x] 2.3 Include `TileRectTypeCdf` in saved subset copy/average handling without claiming real frame-end CDF update support.
...
- [x] 3.2 Expand selector tests for valid `plane_start`/`ctx` edges and typed bounds errors for `TileRectTypeCdf`.
- [x] 3.3 Expand `SymbolDecoder::read_symbol(cdf)` handoff tests to include `TileRectTypeCdf`.
- [x] 3.4 Expand saved copy/average tests so the supported subset includes `TileRectTypeCdf`.
...
- [x] 4.2 Regenerate decoder support/status, feature status, and decoder conformance coverage docs required by repo checks.
...
- [x] 5.2 Run `openspec validate tile-rect-type-cdf-selector --strict`.
- [x] 5.3 Run `cargo xtask feature-status`, `cargo xtask check-feature-status`, `cargo xtask check-decoder-support`, `cargo xtask check-decoder-conformance-coverage`, and `cargo xtask ci`.
