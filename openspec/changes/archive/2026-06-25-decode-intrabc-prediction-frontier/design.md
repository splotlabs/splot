...
The merged local decoder mission selectable-transform path consumes the observed IntrABC
...
does not yet expose a same-workspace copy primitive, and the live local decoder mission LR path
...
- Derive luma IntrABC target/source sample envelopes and subpel prediction
  phase from the retained local decoder mission block geometry and §5.20.7.13 block vector,
...
- Advance the live local decoder mission probe past the generic IntrABC prediction stop to a
...
- Populate decoded `CurrFrame` or `CdefFrame` samples for the full local decoder mission tile.
- Decode chroma IntrABC prediction, residual addition, loop restoration,
  output/reference refresh, or the full local decoder mission stream.
...
2. Keep the live local decoder mission runtime honest about unpopulated samples.

   The decoder should parse IntrABC mode-info as today, derive luma prediction
   geometry, and only dispatch to the workspace copy primitive when a populated
   current-frame workspace is available. The current live local decoder mission path does not
...
- [Risk] A workspace copy API can be misused on unavailable source samples.
  ...
  must not call it in the live local decoder mission path until decoded samples are populated.
...
- [Risk] The scope could drift into full IntrABC reconstruction.
  ...
  restoration, output/reference refresh, and full local decoder mission decode unclaimed.
