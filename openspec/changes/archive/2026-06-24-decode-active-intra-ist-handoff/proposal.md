...
The local decoder mission stream now proves the zero-IST frontier was too small:
...
Feature ID: `DECODE-ACTIVE-INTRA-IST-HANDOFF`.

## What Changes

- Add a policy-scoped residual admission mode for local decoder mission LR tx-skip record
...
- Update the implementation matrix, decoder-support matrix, local decoder mission CLI
...
- `active-intra-ist-handoff`: Tracks the specific active intra IST
  syntax handoff needed for LR tx-skip record derivation while reconstruction
  remains unsupported.

### Modified Capabilities

- `decoder-support`: Records the new partial local decoder mission support row and associated
...
- Affects focused `splot-decode` residual/LR tests and the ignored local decoder mission
