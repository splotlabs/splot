...
The active local decoder mission runtime path is inside the key-frame Wiener NS LR selectable
...
- Prove the local decoder mission probe advances past the current
...
- No broad CfL prediction, MHCCP prediction, chroma reconstruction, decoded
  ...
  local decoder mission success claim.
...
- **Risk:** Incorrect `CflAllowedInSdp` state can still desynchronize symbols.
  **Mitigation:** Add focused traversal tests for a top luma/chroma partition
  combination that sets the value to `0`, and verify the live local decoder mission probe.
