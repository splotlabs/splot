...
The live local decoder mission decode probe now reaches AV2 §5.20.5 intra mode-info inside the
...
- for luma/shared leaves it skips the §5.20.5.3 prelude syntax (`use_intrabc`,
  ...
  partition syntax in the local decoder mission stream.
...
- Add Feature ID `DECODE-SDP-CFL-ALLOWED-FRONTIER` for the local decoder mission SDP
  chroma mode-info prerequisite.
- Add Feature ID `DECODE-INTRA-PRELUDE-TX-FRONTIER` for the local decoder mission
...
- Gate the LR transform-record handoff with the same explicit minimal-tool
  ...
  local decoder mission tools intentionally consumed by this path.
...
- Reject chroma-offset leaves before deriving chroma residual coordinates from
  the wrong luma leaf.
- Add focused tests for the `CflAllowedInSdp` derivation and a live local decoder mission probe
...
- `sdp-cfl-allowed-frontier`: SDP `CflAllowedInSdp` retention and
  chroma mode-info synchronization for the local decoder mission Wiener NS LR transform-record
  path.
- `intra-prelude-tx-frontier`: luma/shared intra mode-info prelude,
  tool-admission, CDEF/delta-Q synchronization, and chroma-offset safety for
  the local decoder mission Wiener NS LR transform-record path.

### Modified Capabilities

- `selectable-transform-records`: records the new SDP `CflAllowedInSdp`
  prerequisite before selectable transform records can continue to decoded
  sample population.
- `decoder-support`: adds the decoder support row for the new partial local decoder mission
  frontier.
...
- Affected code: `crates/splot-decode/src/tile_payload/partition_traversal.rs`,
  ...
  `crates/splot-decode/src/runtime_minimal/wienerns_lr.rs`, and the local decoder mission CLI
  regression.
