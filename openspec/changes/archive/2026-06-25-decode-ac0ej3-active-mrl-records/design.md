...
The merged selectable narrow-record handoff moves the local ac0ej3 probe to
...
- Move the local ac0ej3 probe beyond the active-MRL unsupported diagnostic to
...
2. Return `UsesMrls` through `GeneralIntraLeafMode`.
   ...
   MRL state recording centralized and avoids special-casing the ac0ej3 LR
   callback.
...
4. Keep failure modes structured and narrow.
   ...
   diagnostics tied to the active ac0ej3 matrix rows. The change must not turn a
...
- The next ac0ej3 frontier is not known until the runtime advances past active
