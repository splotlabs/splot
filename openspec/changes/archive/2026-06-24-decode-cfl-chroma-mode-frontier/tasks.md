...
## 3. local decoder mission Runtime Handoff

- [x] 3.1 Thread the typed CfL chroma mode through the local decoder mission selectable-transform record path and coefficient residual handoff.
...
- [x] 4.1 Add the `DECODE-CFL-CHROMA-MODE-FRONTIER` matrix/support rows and refresh generated status docs.
- [x] 4.2 Run focused tests, the local decoder mission decode probe, `openspec validate --all --no-interactive`, `cargo xtask feature-status`, `cargo xtask check-feature-status`, and `cargo xtask ci`.
