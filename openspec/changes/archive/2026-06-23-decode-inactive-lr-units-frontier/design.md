...
The minimal runtime currently parses the local decoder mission key frame through AV2
...
- Keep the implementation matrix and decoder support matrix honest about the
  new narrow Feature ID `DECODE-INACTIVE-LR-UNITS-FRONTIER`.
...
- Adding 10-bit sample storage, output serialization, reference refresh support,
  broad local decoder mission decode success, new public APIs, or new dependencies.
...
- Active LR units could remain the current local decoder mission blocker. Mitigation: the
