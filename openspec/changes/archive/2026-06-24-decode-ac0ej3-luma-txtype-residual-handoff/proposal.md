...
The live ac0ej3 probe now reaches a structured
...
- Add Feature ID `DECODE-AC0EJ3-LUMA-TXTYPE-RESIDUAL-HANDOFF`.
...
- Keep inverse transforms, residual addition, loop-restoration filtering,
  output, reference refresh, AVM/dav2d byte equality, and successful ac0ej3
...
- `ac0ej3-luma-txtype-residual-handoff`: Covers syntax-only LR handoff for
  active non-DCT luma transform types in the local ac0ej3 transform-record
  residual path.

### Modified Capabilities

- `decoder-support`: Records the new partial decoder-support row and the live
  ac0ej3 probe evidence after the former DCT-only frontier advances.
