...
The current local decoder mission LR runtime path can parse frame-level Wiener NS LR syntax,
...
- Add a decoder-private selectable transform-record reader for the supported
  local decoder mission intra LR path.
...
- Populate live LR `LrTxSkip` storage for the local decoder mission stream before the
...
- Apply loop-restoration filters, emit 10-bit output, refresh references, or
  claim AVM/dav2d equality for local decoder mission.
...
- Model selectable transform records inside the decoder runtime instead of
  widening public APIs. The data is only needed to advance the private local decoder mission LR
...
- Fail closed for unsupported transform partition shapes or syntax contexts.
  The parser may support the partition forms needed by local decoder mission, but it must not
...
- Selectable transform syntax is recursive enough to hide off-by-one geometry
  ...
  local decoder mission diagnostic movement through the existing live-grid population
  helper.
