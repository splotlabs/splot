...
The local ac0ej3 mission stream now reaches AV2 transform-tool residual syntax
...
Feature ID: `DECODE-AC0EJ3-INTRA-IST-ZERO-FRONTIER`.
...
- Update implementation and decoder-support tracking plus the local ac0ej3
  runtime gate test.
...
- `ac0ej3-intra-ist-zero-frontier`: Tracks the ac0ej3-specific intra IST
  zero-secondary-transform syntax frontier and its fail-closed active-secondary
  transform boundary.

### Modified Capabilities

- `decoder-support`: Records the new partial ac0ej3 support row and the
...
- Affects `crates/splot-cli/tests/decode_cli.rs` only if the local ac0ej3 probe
...
- No new dependencies, dependency-graph changes, encoder work, successful
  ac0ej3 output claim, AVM/dav2d equivalence claim, reconstruction expansion,
