...
- Add `DECODE-LR-CLASSIFIED-WIENER-VALUES` as the matrix-owned decoder
...
- Add decoder helper state that derives `FilterClass[y >> 2][x >> 2]` values for
  ...
  and `LrTxSkip` values.
- Keep live local decoder mission fail-closed and structured, with a diagnostic that identifies
  missing 10-bit current/CDEF frame storage and missing `LrTxSkip` storage.
- Prove the helper with focused synthetic-value tests and local decoder mission diagnostic
  evidence.

**Non-Goals:**

- No real local decoder mission frame-buffer reads in the live runtime.
...
- Use caller callbacks for source samples and `LrTxSkip` values instead of
  synthetic live values. This prevents the local decoder mission runtime from fabricating
...
- Move the live diagnostic to a new matrix row once the value-capable path is
  ...
  classification over supplied values, but local decoder mission lacks the runtime storage
