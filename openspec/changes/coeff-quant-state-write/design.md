...
- No derivation of `isLf`, `maxLevel`, parity hiding, TCQ enablement, TCQ
  starting state, `Lossless`, or scan tables from real block syntax.
...
3. Model ordinary non-FSC TCQ as caller-selected facts.

   Rationale: the spec updates `tcqState` in both the level-read and quant-write
   phases when `useTcq` is active. This helper can apply the post-`read_quant`
   formula from caller-supplied `useTcq`, `lossless`, and starting `tcqState`
   without deriving those facts from syntax yet.
...
- TCQ state is data-dependent and easy to overclaim -> keep it crate-private,
  caller-selected, and covered by focused tests rather than wiring runtime block
  syntax.
