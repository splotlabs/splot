...
- [x] 4.4 Reject non-zero effective quantizer deltas (`DeltaQ* + Base*DeltaQ`) before the residual path uses the verified zero-delta dequantization subset.
...
- [x] 6.3 The zero-effective-delta admission gate is pinned by `effective_quantizer_delta_gate_includes_frame_and_sequence_offsets`.
