...
The ac0ej3 decoder mission now consumes the observed IntrABC mode-info and
...
- `ac0ej3-selectable-transform-records`: Advance the ac0ej3 IntrABC
...
- `decoder-support`: Track the new `RECON-INTRABC-CURRENT-FRAME-COPY` support
  row and the updated ac0ej3 selectable-transform frontier evidence.
...
- Affected crates: `splot-recon` for the workspace copy primitive and
  `splot-decode` for the ac0ej3 IntrABC runtime handoff.
- Affected docs/tracking: `docs/IMPLEMENTATION-MATRIX.toml`,
  ...
  and the local ac0ej3 CLI probe expectation.
