...
checked integer-vector luma current-frame prediction geometry against the
padded MI-domain current-frame bounds, rejects fractional vectors until the
§7.13.3.18 filter footprint is modeled, and SHALL continue to mark decoded
sample population, loop-restoration filtering/output, reference refresh,
AVM/dav2d byte equality, and successful ac0ej3 decode as unsupported or
unclaimed.
...
#### Scenario: IntrABC fractional prediction frontier is tracked
...
- **THEN** `docs/DECODER-SUPPORT-MATRIX.toml` SHALL list the IntrABC
  ...
  fractional-prediction unsupported reason reached by the local fixture.
