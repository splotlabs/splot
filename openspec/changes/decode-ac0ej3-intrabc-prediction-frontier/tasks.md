...
- [x] 2.1 Add an ac0ej3 IntrABC luma prediction-geometry helper that converts retained eighth-pel block vectors into checked integer-vector target/source rectangles for the current block using padded MI-domain bounds, rejecting overflowing, out-of-frame, or fractional geometry with structured diagnostics.
- [x] 2.2 Wire the selectable-transform IntrABC path to derive prediction geometry after NEARMV/NEWMV block-vector syntax and advance the live local probe to a precise fractional-prediction frontier instead of the generic IntrABC prediction stop, while integer vectors continue to the missing-populated-`CurrFrame` frontier.
...
- [x] 2.4 Add focused `splot-decode` tests for NEWMV geometry, NEARMV geometry, fractional luma prediction rejection, MI-domain edge geometry, out-of-frame rejection, non-IntrABC regression behavior, and the live-path fractional/`CurrFrame` diagnostics.
...
- [x] 3.3 Update the ignored local ac0ej3 CLI probe expectation to the next structured unsupported-feature frontier reached after the IntrABC fractional-prediction guard.
