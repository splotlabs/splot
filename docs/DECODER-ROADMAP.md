...
pAngle 90 and `H_PRED` pAngle 180). General directional angles, full edge
...
| 8 | Scalar intra prediction, dequant/reconstruction, inverse transform, frame hashes | current-frame workspace plus square DC, rectangular DC, subsampled DC, IBP DC, basic/PAETH, smooth, H/V cardinal directional primitives, and the minimal luma-DC plus traced top-left chroma H_PRED runtime handoff supported; general directional angles, DIP/general directional-angle IBP/full CfL modes, broad chroma prediction, dequant/reconstruction, inverse transforms, broad runtime hashes planned |
