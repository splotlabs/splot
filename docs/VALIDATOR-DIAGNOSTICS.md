...
| `frame-header/frame-size-exceeds-sequence-max` | error | § 6.17.4.1 | derived FrameWidth/FrameHeight exceeds active sequence maximum (the explicit-override `frame_size()` path; the `cur_mfh_id > 0` non-override default path, where the derived dims equal the MFH's stored dims, is reported by `frame-header/mfh-frame-size-exceeds-sequence-max` instead — no double-fire) |
...
| `frame-header/mfh-frame-size-exceeds-sequence-max` | error | § 6.17.2 | a `cur_mfh_id > 0` frame references a multi-frame header whose STORED mfh_frame_width/height_minus_1 exceeds max_frame_width/height_minus_1 (mirror :4348-4349). Evaluated after load_sequence_header for every referencing frame, INDEPENDENT of frame_size_override_flag (an in-range override hides an out-of-range MFH), so distinct from the derived-FrameWidth `frame-header/frame-size-exceeds-sequence-max`. An MFH with no mfh_frame_size payload infers its dims to the maxima (§ 5.18.2, mirror :4101) and stays silent; an unresolvable MFH stays silent. On the override==0 default path (derived dims == stored dims) this rule is the single home and the §6.17.4.1 derived check defers |
