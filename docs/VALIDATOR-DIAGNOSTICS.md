...
| `celu/doh-order-hint-bits-mismatch` | error | § 7.3.7 | with a DOH constraint flag set, frame units in one temporal unit carry different OrderHintBits (mirror line 655). The accumulator is fed per frame UNIT, not per OBU (F1): an `OpensNewUnit` OBU notes its unit's resolved bits (or None when its opener does not resolve, still soundly poisoning the unit's unknowable bits), an `Ambiguous` OBU notes None (it might open a unit with unknowable bits), and a `ContinuesUnit` OBU (a non-first tile group of an already-counted coded frame) is skipped — so a continuation no longer contributes a redundant None that would poison the whole temporal unit's judgment and over-suppress a real mismatch. A frame contributes its OrderHintBits only when its referenced sequence header resolved to the active header it parsed against (the stale-activation guard); a frame referencing an absent / stale header contributes None (which drops the judgment) rather than the stale active header's bits. A global frame-bearing OBU (obu_xlayer_id == GLOBAL_XLAYER_ID, itself invalid and diagnosed by `obu-header/global-xlayer-allowed-types`) is excluded before the accumulator — it is not part of any CELU (§ 7.3.6) and never resolves an active header, so it no longer feeds a spurious None that would poison the temporal unit's judgment and over-suppress a real mismatch between the valid CELUs |
| `celu/doh-order-hint-mismatch` | error | § 7.3.7 | with a DOH constraint flag set, coded output frame units in different coded extended layer units of one temporal unit carry different OrderHint (mirror lines 656-657 / 1316-1320). Compared via the `order_hint` LSB **proxy** for the decoded OrderHint (`get_disp_order_hint`; a named residual blocked on reference-state modelling — AV2-5.18.2-FRAME-HEADER-INFO). Because equal decoded OrderHints can carry different-width LSB encodings, the cross-CELU comparison is **gated on the temporal unit's OrderHintBits being known and equal**; when they differ, `celu/doh-order-hint-bits-mismatch` fires instead and this comparison drops. The DOH flag for a completed temporal unit (shared with `celu/doh-order-hint-bits-mismatch`) is the `multistream_doh_constraint_flag` of the preceding MSDO **or** the `lcr_doh_constraint_flag` of the activated global LCR of the CMVS that **contains** that temporal unit (§ 7.3.7 "for each temporal unit in the CMVS"). The LCR side is sampled against that GOVERNING CMVS window — captured before the § 7.3.2 begin/end conditions mutate the live window — so a CLK boundary temporal unit that ENDS the CMVS (end condition 2, clearing the live window) is still governed by the ending CMVS's activated global LCR rather than reading the flag false (the MSDO side is window-independent live last-wins state) |
| `celu/in-unit-order` | error | § 7.3.6 | an HLS header appears after a later HLS-header phase (LCR → OPS → atlas → sequence header), or a frame unit opens at a lower obu_mlayer_id than an earlier one, or a coded frame unit opens in an embedded layer after that layer's coded output frame unit (the per-layer output-slot grammar: zero or more non-output units then zero or one output unit, the output unit last — mirror lines 521-529). The ascending-obu_mlayer_id ordering counts every frame-unit constituent at its own obu_mlayer_id — the head (CI) and interior (BRT/QM/FGM/prefix-metadata/MFH) OBUs as well as the coded frame — so a head heading a higher embedded layer's unit makes a later lower-mlayer coded frame out of order (a suffix metadata shares its frame's mlayer, so it never lowers the high-water mark). The ordering is boundary-independent (F3): a `ContinuesUnit` frame OBU (a non-first tile group) belongs to its opener's already-begun unit and an `Ambiguous` frame OBU belongs to some layer-m unit either way, so both participate in the ascending-mlayer accounting — e.g. mlayer0 first tile, mlayer1 frame, mlayer0 non-first tile fires. Coded-frame-unit boundaries come from the FrameUnitSegmenter (the single source of truth); an Ambiguous boundary drops the per-layer output-slot judgment (but not the boundary-independent ascending-mlayer one). Disjoint from `obu-order/non-global-hls-before-coded-layer` (the HLS-header-after-frame-region case) |
