...
| `celu/doh-order-hint-bits-mismatch` | error | § 7.3.7 | with a DOH constraint flag set, frame units in one temporal unit carry different OrderHintBits (mirror line 655). A frame contributes its OrderHintBits only when its referenced sequence header resolved to the active header it parsed against (the stale-activation guard); a frame referencing an absent / MFH-associated / stale header contributes None (which drops the judgment) rather than the stale active header's bits |
