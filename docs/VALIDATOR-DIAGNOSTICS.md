...
| `annex-a/msdo-prohibited-for-iop` | error | § A.2 | a Table A.4 interoperability-point row prohibits an OBU_MSDO (a single-extended-layer coded video sequence) but one is present. Under the strict Table A.3 definition (mirror lines 146-151) a present OBU_MSDO declares num_streams_minus_2 + 2 >= 2 extended layers, so E > 1 and this row is in-band unreachable whenever an MSDO is actually present; it is retained as the defensive arm and does NOT fire on an observed single-xlayer count overriding the MSDO's declared count. The declared-vs-observed reconciliation (an MSDO declaring substreams that never materialize) is owned by the upcoming msdo-substream-constraint-checks change. |
