...
| Rule ID | Severity | Section | Condition |
|---|---|---|---|
| `celu/clk-olk-mixed` | error | § 7.3.6 | a coded extended layer unit contains both an OBU_CLOSED_LOOP_KEY and an OBU_OPEN_LOOP_KEY (mirror line 554). CLK/OLK identity is type-decided, so it is recorded even for an Ambiguous-boundary OBU — an OLK plus an ambiguous CLK still fires |
...
| `celu/doh-order-hint-bits-mismatch` | error | § 7.3.7 | with a DOH constraint flag set, frame units in one temporal unit carry different OrderHintBits (mirror line 655). The accumulator is fed per frame UNIT, not per OBU (F1): an `OpensNewUnit` OBU notes its unit's resolved bits (or None when its opener does not resolve, still soundly poisoning the unit's unknowable bits), an `Ambiguous` OBU notes None (it might open a unit with unknowable bits), and a `ContinuesUnit` OBU (a non-first tile group of an already-counted coded frame) is skipped — so a continuation no longer contributes a redundant None that would poison the whole temporal unit's judgment and over-suppress a real mismatch. A frame contributes its OrderHintBits only when its referenced sequence header resolved to the active header it parsed against (the stale-activation guard); a frame referencing an absent / stale header contributes None (which drops the judgment) rather than the stale active header's bits |
| `celu/doh-order-hint-mismatch` | error | § 7.3.7 | with a DOH constraint flag set, coded output frame units in different coded extended layer units of one temporal unit carry different OrderHint (mirror lines 656-657 / 1316-1320). Compared via the `order_hint` LSB **proxy** for the decoded OrderHint (`get_disp_order_hint`; a named residual blocked on reference-state modelling — AV2-5.18.2-FRAME-HEADER-INFO). Because equal decoded OrderHints can carry different-width LSB encodings, the cross-CELU comparison is **gated on the temporal unit's OrderHintBits being known and equal**; when they differ, `celu/doh-order-hint-bits-mismatch` fires instead and this comparison drops |
