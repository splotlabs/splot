...
| `frame-header/ref-frame-idx-invalid-slot` | error | § 6.17.2 | an inter frame's `ref_frame_idx[i]` (explicit reference map, §5.18.2 mirror :4611-4625) names a reference slot the modeled §7.23 reference state proves invalid (`RefValid[idx] == 0` against a modeled per-extended-layer buffer), or an index at/beyond the NUM_REF_FRAMES buffer. Slots the model has not grounded stay Unknown and are not reported (no guessing); the under-reported direction is the implicit reference map (`get_ref_frames()`), which needs unmodeled derivation |
