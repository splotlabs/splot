...
- Land private block-symbol emission incrementally before the tile body, starting
  ...
  eob_pt). The `sec_tx_type` IST token (the §5.20.8.2 secondary-transform symbol read right
  after intra_tx_type) now exists; a trace inserting it, the most_probable_stx_set
  follow-up, blocks with eob>2, the chroma base-range/golomb tiers, partition syntax,
