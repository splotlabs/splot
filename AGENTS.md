...
cargo +nightly fuzz run parse_obu   # full local fuzz run of one target (nightly-only; `cargo install cargo-fuzz --locked`).
                                    # Targets: parse_obu, validate_bytes, parse_ivf, parse_bitstream, symbol_decoder_bytes, tile_payload_decode_bytes, decode_plan_bytes, decode_runtime_hash_bytes, decode_runtime_y4m_bytes, recon_frame_hash_bytes, recon_y4m_output_bytes, recon_intra_prediction_bytes (`cargo +nightly fuzz list`).
